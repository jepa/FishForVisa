...
![Figure 2. Weighted benefits of transboundary species by $km^{2}$ and UN sub-region. A) Units in thousand 2019 USD. B) Units in tonnes. Points are sub-region averages $\pm$ s.d. by country](../Figures/Fig2.png)

**We determined the catch trend of each species within each EEZ (Fig. 3). While previous studies demonstrated that catch trends can be used to infer whether a stock is healthy, re-building, over-exploited or collapsed [@Kleisner:2011wn], several factors can influence stock status. Therefore, our intention here is to examine where the catch trends of transboundary species are different from non-transboundary species between 2005 and 2010 relative to historic catch since 1951 (See methods).** Catch trends are classified as increasing (Category A), constant (Category B) or decreasing (Category C). We found significant differences within all catch trend categories for transboundary species (one-way ANOVA, *Df*(2,459) = 47.94, *p* < 0.001, $\alpha$ = 0.05; Tukey's post hoc test *p* $\leq$ 0.001) as opposite to discrete species where no significant differences where registered (one-way ANOVA, *Df*(2,106) = 1.885, *p* = 0.157; $\alpha$ = 0.05). We also found significant differences in catch trends when directly comparing transboundary to discrete species (MANOVA, *Df*(2,459) = 19.001, *p* < 0.001). **Overall, transboundary species that are only fished by one country are generally less likely to have a decreasing catch trend compared to when the species is shared by neighboring countries (Supplementary Table S2)**. 
...
![Figure 3. Number of transboundary species by catch trend and EEZ. Category A, Growing trend; Category B, Constant trend; Category C, Shrinking trend](../Figures/Fig3.png)
...
We estimated the economic contribution in **2019** real USD of transboundary species for each country using global ex-vessel price data [@Tai2017]. The database we draw from includes ex-vessel price derived from multiple sources and a structured interpolation method (e.g. similar countries, species) to fill in data gaps [@Sumaila:2015uc]. **The dataset is harmonized with the SAU catch data to estimate yearly fishing revenue (as ex-vessel price) for all species and EEZs of this study (http://www.seaaroundus.org/data/#/feru). We report the average fishing revenue of 2005 to 2014 derived from the fishing activity within the global EEZs. We did not include revenue from areas beyond national jurisdiction. We used the US Gross Domestic Product Implicit Price Deflator (average of all 2019 quarters) according to the US Saint Louis Federal Reserve Bank (https://fred.stlouisfed.org/searchresults?st=GDP+Implicit+price+deflator) to standardize the original 2010 real USD value to 2019 real USD.**
