...
```{r Call_Sup_1, eval = T, fig.width=5, fig.height=3, echo=FALSE, fig.pos= "H", fig.cap="Number of transboundary species per EEZ and their contributions to countries’ EEZ catch. The number of species is displayed in the EEZ while catch (in thousand tons) is displayed in the land polygon."}
...
```{r Call_Sup_4, eval = T, fig.width=6, fig.height=4, echo=FALSE, fig.pos= "H", fig.cap=" Histogram of number of transboundary species using different Area Index threshold values. The darker histogram represents the threshold (25%) used in the present analysis"}
...
```{r Call_SupT_2, eval = T, echo = F, fig.pos= "H"}

read.csv("~/GitHub/FishForVisa/Tables/Table_S2.csv") %>% 
  kable("latex",
        booktabs = T,
        caption= "Average number of transboundary and discrete species per EEZ for each catch trend")
