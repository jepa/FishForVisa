...
# Get the area of each eez provided in the shapefile 
EEZ_Area <- World_sf %>% 
  as.data.frame() %>% 
  group_by(eez_id) %>% 
...
### Data for land polygon
# Total_Value_Entity <- Clean_sau_eez_fes %>% 
#   group_by(fishing_entity_id, fishing_entity) %>% 
...
Value_Trans_Country <- Tresholded_T_Data %>% 
  ...
  group_by(fishing_entity_id,Variable) %>% # Estimate the amount of dollars of each nation 
  summarise(
