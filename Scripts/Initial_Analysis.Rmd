...
## Determine Catch Trends

This sub routine determines the species exploitation trend based on the SAU methodology. **Note:** For the current project we do not consider this method to determine the species exploitation trend *per se*, but rather a catch trend that *could* indicate that a species is under/over/max exploited.
...
Once those thresholds are met, the trend of the species is determined as follows:
...
Finally we choose the reporting year based on the most frequent trend in the last 10 years of data. In cases where there was a mach between trend (e.g. Cayman Islands for species 6000006 had 5 *Max Exploited* and 5 *Overexploted*) then we report the trend at the last year of data.
...
### Function `EstCatchTrend`

This function estimates the exploitation trend of a species based on catch data.

```{r funEstCatchTrend, eval = T, echo =F}

# Spp <- 601417
# Coord = CoorG

EstCatchTrend <- function(Spp,Coord, Save_Path = "NA", Per_Taxa = NA){
  ...
  # Tresholds to evaluate stocks if these are not met, then soecies Trend = "No Trend"
  Tresholds_Data <- Data %>% 
    arrange(Territory) %>% 
  ...
  # Get Variables to perform Trend metric
  Trend_Data <- Data %>% 
    # filter(Territory == "Israel (Red Sea)") %>%  # For testing
    anti_join(No_Trend_Data, # Removes no Trend data
    ...
    # Final Catch Trend Evaluation 
    mutate(Trend = ifelse(Year > Y_Post_Max_Min & Post_Max_Min < (Max_Peak*0.10) & ((RMean > (Max_Peak*0.10) & RMean < (Max_Peak*0.50))),"Rebuilding",
  ...
  if(Per_Taxa == T){
    ...
    Trend_Data <- Trend_Data %>%
      ungroup() %>%
      select(TaxonKey,Territory,Year, Trend)
    ...
    Name <- paste(Save_Path,Spp,"_Trend.csv",sep="")
    
    # Save file for each species
    write_csv(Trend_Data,
              Name)
  }else{
    
    ### Determine the final Trend using the top category over the last 10 years
    
    Determine_Trend_Data <- Trend_Data %>% 
      filter(Year > 2004) %>% # last 10 years of data (2005-2014)
      group_by(TaxonKey,Territory,Trend) %>%
      summarise(n_Trend= n()) %>%
      group_by(TaxonKey,Territory) %>%
      top_n(1,n_Trend)
    ...
    suppressMessages( # supresses "No duplicate combinations found of: Territ"
      Duplicates <- Determine_Trend_Data %>% 
        ungroup() %>% 
    ...
    Duplicates_Trend <- Trend_Data %>% 
      filter(Territory %in% Duplicates,
             Year == 2014) %>%
      ungroup() %>%
      select(TaxonKey,Territory,Trend)
    
    # Build Final Trend database
    Final_Trend_Data <- Determine_Trend_Data %>% 
      filter(!Territory %in% Duplicates) %>% 
      bind_rows(Duplicates_Trend)
    
    # If both datasets have value RETURN BOTH
    if(nrow(No_Trend_Data) > 0 & nrow(Final_Trend_Data) > 0){
      
      Fun_Result <- bind_rows(No_Trend_Data,
                              Final_Trend_Data)
    }
    
    # If only No trend data, then just return No Trend Data
    if(nrow(No_Trend_Data) > 0 & nrow(Final_Trend_Data) == 0){
...
# Testing
# Test_1 <- EstSppTrend(Species_List[1],CoorG)

```

### Data Catch Trend

### Sub routine for Catch Trend

```{r Spp_Trend_Control_Pannel, eval = T, echo = F}
...
```{r Spp_trend_Sub_Routine, eval = T, echo = F}
# Needs to have trans species 
Species_Trend <- bind_rows(
  mclapply(
    Species_List[1:2],
    FUN = EstCatchTrend,
    Coord = CoorG,  # Gabriels coordinate system for read species dist
    Per_Taxa = T,
    Save_Path = "Z:/Vicky/Stock_Trend/"
  )
)

# Save data
readr::write_csv(Catch_Trend,
                 paste(Results_Path,"Species_Trend.csv",sep="")
)
# head(Species_Trend)
```
...
StraddIndex <- function(Spp,Model = "All",Neighbours,Coord,Index_Code){
  ...
  if("fao" %in% Trans_Spp$category == FALSE){
    ...
    Save_Path <- my_path("R","EEZ/Stradling_Results/", File_Name)
...
fao_to_sau <- read_excel("/Volumes/DATA/JULIANO_NEYMAR/Spatial/SAU/EEZ-FAO-Country-Combo.xlsx") %>% 
  select(territory = eez_name,eez_id,fao_area_name)
...
neighbours <- read_excel("/Volumes/DATA/JULIANO_NEYMAR/FishForVisa/Data/Spatial/EEZ_Neighbour_List.xlsx") %>% 
  ...
  left_join(fao_to_sau,
            by = "eez_id") %>% 
  select(territory = territory.x,neighbour)
...
depth_profile <- my_path("G","FishForVisa/Data/Distribution/Species","SppTaxonName.csv", read = T)
...
suppressWarnings(
  suppressMessages(
    lapply(unique(transboundary_spp$taxon_key), StraddIndex, Model = "All", Coord = coords,
           Index_Code = Index_Code)
  )
)
...
GetTransResults=function(Spp, type = "NA"){
  
  if(type == "strad"){
    # Set the path for each file
    Distpath <- paste(my_path("R","EEZ/Straddling_Results/"),Spp,"_straddling.csv",sep="")
...
CreateFigureOne <- function(Variable_Selected){
  ...
  if(Variable_Selected == "catch"){
    Map_Label <- "Catch (Thousand Tonnes)"
  ...
  World_sf_si_land <- World_land_sf %>%
    ...
    left_join(Value_Trans_Country, by ="fishing_entity") %>% 
    filter(Variable %in% Variable_Selected)
  
  # Get the NA's to be grey
  Grey_land <- World_land_sf %>%  
    filter(!fishing_entity %in% World_sf_si_land$fishing_entity)
  ...
  Labels_Trans <- c("(1-29]","(29-57]","(57-85]","(85-113]","(113-141]")
  # Labels <- c("(1-10]",paste(Bins[2:5]))
  Labels <- c("(1-10]","(10-100]","(100-1000]","(1000-5000]","(5000-12000]")
...
UN_Regions <- read.csv(paste(my_path("D"),"Spatial/UN_Regions.csv",sep=""), header = TRUE) %>% 
  ...
  select(-n) %>% 
  filter(fishing_entity != "") %>%  # there are three rows with "developing" trend as in, the code
  ungroup() %>% 
...
Remove highly migratory species according to UNCLOS extracted from FAO (Maguire et all). The above tuna species listed as highly migratory species in Annex 1 of UNCLOS have extensive distribution on the high seas

Maguire, J-J., Sissenwine, M., Csirke, J., Grainger, R., & Garcia, S. (2006). The state of world highly migratory, straddling and other high seas fishery resources and associated species. FAO.

```{r highly_migratory_species}

high_mig_taxa <- c("Thunnus alalunga", "Thunnus thynnus","Thunnus orientalis","Thunnus obesus","Katsuwonus pelamis", "Thunnus albacares","Thunnus atlanticus","Euthynnus alletteratus","Euthynnus affinis","Thunnus maccoyii", "Auxis thazard", "Auxis rochei",# Tuna-likes
                   "Tetrapturus angustirostris", "Tetrapturus belone", "Tetrapturus pfluegeri", "Tetrapturus albidus", "Tetrapturus audax", "Tetrapturus georgei", "Makaira indica", "Makaira nigricans","Istiophorus platypterus", "Istiophorus albicans","Xiphias gladius")

length(high_mig_taxa) # 23



high_mig_spp <- my_path("D","Distribution/Species","SppTaxonName.csv", read = T) %>% 
  filter(taxon_name %in% high_mig_taxa)

nrow(high_mig_spp) # 21

# ---------------- 
# Missing 4 species
# missing_hm_spp <- as.data.frame(x = high_mig_taxa) %>% 
# filter(!high_mig_taxa %in% high_mig_spp$taxon_name)


# Tetrapturus belone # only found in the med				
# Tetrapturus georgei # Misspselled, correct name Tetrapturus georgii but not identified in the SAU dataset
...
# Get species taxon data
taxon_list <- my_path("G","FishForVisa/Data/Distribution/Species","SppTaxonName.csv", read = T)


# Load straddling species
# Spp list
spp_complete <- list.files(my_path("R","EEZ/Straddling_Results/"))

stradling_spp <- str_replace(spp_complete,"\\_.*","")

stradd_data <- bind_rows(
  lapply(stradling_spp,GetTransResults, type = "strad")
) %>% 
  left_join(taxon_list) %>% 
  select(taxon_key,taxon_name,dem_pel, everything()) %>% 
  mutate(category = ifelse(taxon_key %in% high_mig_spp$taxon_key,"high_migratory","straddling"))


# Complete transboundary data

stradd_hs_spp <- stradd_data %>% 
  select(taxon_key,category,sf_eez_name = territory, eez_neighbour = neighbour) %>% 
  unique() %>% 
  left_join(matching_names,
            by = "sf_eez_name") %>% 
  select(1:4,fishing_entity_id)

# STRADDLING: Dataset to match all SAU names
...
# STRADDLING
Clean_Results_Trans <- my_path("R","EEZ","Clean_Results_Trans.csv", read = T) %>% 
  rename(sf_eez_name = eez_name) %>% 
  left_join(matching_names)
...
# Clean_Results_Trans %>%
#   left_join(Exploited_Species) %>%
#   select(1,taxon_name,2:5,fishing_entity,un_sub_region) %>%
#   write_csv(.,"Clean_Results_Trans.csv")
...
# CPI in 2019/ CPI in 2010 × 2010  value = 2019  value

# Used for STRADDLING too
...
# Read Scatch trend Data (Sub-routine Catch Trend)
Trend_Data <- fread(paste(my_path("R"),"EEZ/Species_Trend.csv", sep ="")) %>% 
  clean_names() %>% 
...
Tresholded_T_Data <- Clean_Results_Trans %>% 
  ...
  ## Adding STRADLING component
  left_join(stradd_hs_spp) %>% 
  mutate(category = replace_na(category,"transboundary"))

### Duble check for NA's
# map(Tresholded_T_Data, ~sum(is.na(.))) # No NA's

# shared_classification <- shared_species %>% 
# select(taxon_key,category,fishing_entity_id) %>% 
# unique()
...
# Number of species that are straddling or highly migratory
stradd_data %>% 
  group_by(category) %>% 
  summarize(
    n_spp = length(unique(taxon_key)),
    prop = round((length(unique(taxon_key))/633)*100)
  )

# RESULT
# category  n_spp   prop of transboundary
# high_mig	 20	     3		
# stradd	   84	    13	
# total     104     16


stradd_hs_fentity <- stradd_hs_spp %>% 
  group_by(taxon_key,category, fishing_entity_id) %>% 
  tally() %>% 
  select(-n)

# Econ value WITHOUT HIGH SEAS!!!
Summary_Stradd <-
  SAU_Sampled_Data %>% 
  semi_join(Tresholded_T_Data,
            by = c("taxon_key","fishing_entity_id")
  ) %>% 
  left_join(stradd_hs_fentity,
            by = c("taxon_key","fishing_entity_id")
  ) %>%
  mutate(Category = ifelse(is.na(category),"trans",category)) %>%
  group_by(Variable,Category) %>%
  summarise(Category_Total = sum(Value, na.rm=T))

Summary_Stradd %>% 
  group_by(Variable) %>% 
  summarise(Total_Trans = sum(Category_Total)) %>% 
  left_join(Summary_Stradd,
            by = "Variable") %>% 
  mutate(
    percentages = (Category_Total/Total_Trans)*100
  )


# Econ value WITH HIGH SEAS!!!

# Used for STRADDLING too
SAU_hs_Data <- bind_rows(
  my_path("G","SAU/SAU HighSeas 18,48,34,27,21,47 v48-0","SAU HighSeas 18,48,34,27,21,47 v48-0.csv",read = T),
  my_path("G","SAU/SAU HighSeas 41,31,58,57,51,88 v48-0","SAU HighSeas 41,31,58,57,51,88 v48-0.csv",read = T),
  my_path("G","SAU/SAU HighSeas 77,67,61,87,81,71 v48-0","SAU HighSeas 77,67,61,87,81,71 v48-0.csv",read = T)
) %>% 
  rename(taxon_name = scientific_name) %>% 
  left_join(taxon_list) %>% 
  filter(
    taxon_key %in% stradd_hs_spp$taxon_key,
    year >= 2005 & year <= 2014,
    catch_type == "Landings",
    reporting_status == "Reported"
  ) %>% 
  # aggregate all gears
  group_by(area_name,fishing_entity,taxon_key,taxon_name,year) %>%
  summarise_at(vars("tonnes","landed_value"),
               .funs = sum,na.rm = T) %>% 
  group_by(area_name,fishing_entity,taxon_key) %>% 
  summarise_at(vars("tonnes","landed_value"),
               .funs = mean,na.rm = T) %>% 
    #filter only straddling species
  semi_join(stradd_hs_fentity,
            by = c("taxon_key")
  ) %>% 
  left_join(stradd_hs_fentity_x) %>% 
  group_by(area_name,fishing_entity,category) %>% 
  summarise_at(vars("tonnes","landed_value"),
               .funs = sum,na.rm = T) %>% 
  gather("variable","value",tonnes:landed_value)



stradd_hs_fentity_x <- stradd_hs_fentity %>% 
  group_by(taxon_key,category) %>% 
  tally()
...
SAU_hs_Data %>% 
  ...
  mutate(val_million = value/1000000)
...
plot_eez <- stradd_data %>% 
  group_by(eez_name = territory,category) %>% 
  summarise(
    n_species = length(unique(taxon_key))
  )

plot_fao <- stradd_data %>% 
  group_by(fao_name,category) %>% 
  summarise(
    n_species = length(unique(taxon_key))
  )
...
fao_regions <- my_sf("FAO") %>% 
  filter(F_LEVEL == "MAJOR") %>% 
  select(fao_name= NAME_EN,geometry = the_geom) %>% 
  left_join(plot_fao) %>% 
  filter(!is.na(category))

eez_regions <- World_sf %>% 
  left_join(plot_eez) %>% 
  filter(!is.na(category))


st_crs(fao_regions) = 4326
fao_regions <- st_transform(fao_regions, crs = "+proj=eck4")

plot <-ggplot() +
  geom_sf(data = fao_regions, aes(color = n_species, fill = n_species)) +
  geom_sf(data = eez_regions, aes(fill = n_species), color = "grey30") +
  geom_sf(data = World_land_sf, aes(), fill = "grey") +
  scale_fill_viridis() +
  scale_color_viridis() +
  facet_wrap(~category) +
...
ggsave(
  ...
  height = 15,
  units = "in",
  filename = "Figures/stradling_maps.png"
)
...
936/length(unique(SAU_Global_Data$taxon_key)) # 67.5
...
## Fig. 2 - Weighted bar plots

### Data for Weighted plot
```{r Weight_Revenue, eval =F, echo =F}

#--------------------------- #
### Create Suplemental Table S1
#--------------------------- #

Subregion_Trans_raw_Value %>% 
  select(sub_region,Size_Entity_sum) %>% 
  distinct(sub_region,.keep_all = TRUE) %>% 
...
```{r Trend_Plot_Data, eval = F, echo = F}

# -------------------------- #
# Transboundary data
# -------------------------- #
# Stock trend Data
Trend_Data_Plot <- Tresholded_T_Data %>% 
  left_join(Trend_Data#, 
            # by =c("eez_name","taxon_key","sf_object_id")
  ) %>%
  mutate(trend = ifelse(is.na(status),"No Trend",status)) %>% 
  group_by(fishing_entity,trend) %>% 
  ...
  select(individual = fishing_entity,observation = trend,group = Georegion,value) %>% 
  arrange(group,value) %>% 
...
#### Convert trends to percentage for plot

Totals_trend <- Trend_Data_Plot %>% 
  group_by(individual) %>% 
  summarise(total_trend = sum(abs(value)))

Trend_Data_Plot <- Trend_Data_Plot %>% 
  left_join(Totals_trend) %>% 
  mutate(
    value = round(value/total_trend*100)
  )


length(unique(Trend_Data_Plot$individual))
length(unique(Trend_Data_Plot$observation))

Trend_Plot_Data <- tibble(
  fishing_entity = rep(unique(Trend_Data_Plot$individual),4),
  observation = rep(unique(Trend_Data_Plot$observation),157)
) %>% 
  left_join(UN_Regions) %>% 
  rename(individual = fishing_entity) %>% 
  full_join(Trend_Data_Plot,
...
Exclusive_Discrete_Trend_Data <- Discrete_Exclusive %>% 
  left_join(Trend_Data, 
            by =c("eez_name","taxon_key")
  ) %>%
...
Char <- as.factor(Trend_Plot_Data$group) # Set groups to factors


##### COPY PASTE

# Transform data in a tidy format (long format)
# data <- Trend_Data_Plot

# Set a number of 'empty bar' to add at the end of each group
empty_bar <- 2
nObsType <- nlevels(as.factor(Trend_Plot_Data$observation))
to_add <- data.frame(matrix(NA, empty_bar*nlevels(Char)*nObsType, ncol(Trend_Plot_Data)) )
colnames(to_add) <- colnames(Trend_Plot_Data)
to_add$group <- rep(levels(Char), each=empty_bar*nObsType)
Trend_Plot_Data <- rbind(Trend_Plot_Data, to_add) # Problem converts to list
# Trend_data <- bind_rows(data,to_add)
Trend_Plot_Data <- Trend_Plot_Data %>% arrange(group, individual)
Trend_Plot_Data$id <- rep( seq(1, nrow(Trend_Plot_Data)/nObsType) , each=nObsType)

# to_add$value <- NA
# to_add$observation <- NA
# to_add$individual <- to_add$group

# Trend_data <- Trend_Data_Plot %>%
#   bind_rows(to_add) %>%
#   ungroup() %>%
...
ggsave(
  ...
  units = "in",
  # filename = "Plot.png"
  filename = "Territory_circular_trend.png"
)


```

### Statistical differences between Catch Trend
...
# Transboundary Data
Transboundary_Data_Stats <- Trend_Plot_Data %>% 
  filter(observation != "No Category") %>%
  mutate(value = abs(value))

# Discrete Data (trans.not shared)
Trans_Dis_Data_Stats <- Discrete_Trend_Data %>% 
  filter(observation != "No Category") %>%
  mutate(value = abs(value))

# Discrete Data (just desrete and not trans.not shared)
Discrete_Data_Stats <- Exclusive_Discrete_Trend_Data %>% 
  filter(observation != "No Category") %>%
  mutate(value = abs(value))
...
Manova_Differences <- Transboundary_Data_Stats %>% 
  ...
  left_join(Trans_Dis_Data_Stats, by = c("individual","observation","group")) %>% 
  rename(Trans_Discrete_value= value) %>% 
  mutate(Trans_Discrete_value = ifelse(is.na(Trans_Discrete_value),0,Trans_Discrete_value))
...
mano <- manova(cbind(Discrete_value,Trans_value,Trans_Discrete_value) ~ observation, data = Manova_Differences)
...
# 
#             Df  Pillai approx F num Df den Df    Pr(>F)    
# observation   2 0.17329   21.771      4    918 < 2.2e-16 ***
# Residuals   459                                             
# ---
# Signif. codes:  0 ‘***’ 0.001 ‘**’ 0.01 ‘*’ 0.05 ‘.’ 0.1 ‘ ’ 1
...
# New only comparing shared trans to discrete exclusive
# Kruskal-Wallis chi-squared = 50.723, df = 13, p-value = 2.24e-06
...
# Just country names
country_names <- matching_names %>% 
  select(fishing_entity=sau_fishing_entity,fishing_entity_id) %>% 
  distinct()
