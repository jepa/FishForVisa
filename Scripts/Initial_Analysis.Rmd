...
Based on our assessment (see Methods), 590 transboundary species were also identified as "discrete" species, meaning that the same species could be shared by two neighboring EEZs (transboundary) but not with a third neighbor (discrete).
...
Total_Value_EEZ <- Clean_sau_eez_fes %>% 
...
Value_Trans_Country <- Clean_sau_eez_fes %>% 
  group_by(fishing_entity_id, fishing_entity) %>% 
  summarise(n=n()) %>% # Get all of the fishing entity and id
  select(-n) %>% 
...
r
Continent_Trans_Value <- Subregion_Trans_raw_Value %>% 
  left_join(Regional_Area,
            by = "sub_region") %>% 
  mutate(
    Value_sum= Value_sum*1000000,
    weighted_value = Value_sum/Size_Region
    )
