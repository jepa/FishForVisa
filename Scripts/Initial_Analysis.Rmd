                                                                                             ...
                                                                                             ifelse(eez_name == "South Africa (Atlantic and Cape)","South Africa (Atlantic Coast)",
                                                                                                    ifelse(eez_name =="Viet Nam","Vietnam",
                                                                                                           ifelse(eez_name =="US Virgin Isl.","US Virgin Islands",
                                                                                                                  ifelse(eez_name == "Spain (mainland, Med and Gulf of Cadiz)", "Spain",
                                                                                                                         paste(eez_name)
                                                                                                                  )
                                                                                                           )
                                                                                                    )
...
# New dara produced by Vicky 
sau_eez_fes <- read_excel("~/Desktop/EEZ_FishingEntityID_v2_9June2020.xlsx")
...
SAU_Tim_Data <- fread(paste(my_path("D"),"Distribution/sau_catch_value_country_taxon_JEPA.csv", sep ="")) %>%
  filter(!fishing_entity_id %in% c(213,223,207,208)) %>%  # Remove high seas and 207 and 208 that we do not know 
...
Clean_Results_Trans <- fread(paste(my_path("R"),"/EEZ/Clean_Results_Trans.csv", sep ="")) %>% 
  ...
  fishing_entity = ifelse(fishing_entity == "Cote dIvoire","Côte d'Ivoire",
                          ifelse(fishing_entity == "Curacao (Netherlands)","Curaçao (Netherlands)",
                                 ifelse(eez_name == "Spain (mainland, Med and Gulf of Cadiz)","Spain",fishing_entity)))
  )
...
Total_Catch_Spp <- Total_SAU_Sampled$Total_Sampled[1]/Total_SAU_id$Total_id[1] *100 #96.52%

# Percentage species analyzed from the identified species
Total_spp_analyzed <-Total_SAU_Sampled$Sampled_nspp[1]/Total_SAU_id$id_nspp[1]*100 #63.22%
...
Summary_Trans <- Clean_sau_eez_fes %>%
  ...
  ) %>% # only counting the catch from transboundary speices (e.g. anchoveta from chile and peru but not from equador)
  group_by(Variable) %>%
...
Summary_Trans_vicky <- Tresholded_T_Data %>% 
  ...
  left_join(Clean_sau_eez_fes,
            by = "eez_name") %>% 
  group_by(taxon_key,fishing_entity_id) %>% 
  summarise(n()) %>% 
  left_join(SAU_Global_Vicky,
            by = c("taxon_key","fishing_entity_id")) %>% 
  # filter(eezid != 0) %>% # I think they used high seas data as well?
  group_by() %>%
...
Summary_Discrete <- Clean_sau_eez_fes %>%
  group_by(fishing_entity,fishing_entity_id) %>% 
  summarise(n=n()) %>% 
  left_join(SAU_Sampled_Data) %>% 
  semi_join(Discrete,
  ...
  group_by(Variable) %>%
  summarise(Total_Dis = sum(Value, na.rm=T)) %>% 
  left_join(Total_SAU_Sampled) %>% 
  left_join(Total_SAU_Global) %>% 
  mutate(Per_sampled = Total_Dis/Total_Sampled*100,
         Per_SAU = Total_Dis/Total_Global*100)
...
Dis <- Summary_Discrete %>% 
  mutate(Type = "Dis")

Solo <- Summary_Discrete_Exclusive %>% 
  mutate(Type = "Solo")


All <- bind_rows(Tra,Dis,Solo) %>% 
  select(1:7) %>% 
  group_by(taxon_key,fishing_entity) %>% 
  summarise(n())


missing <- anti_join(Total_SAU_Sampled,All,
