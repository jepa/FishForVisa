...
Create a table that matches all names

```{r names_table, eval =F, echo =F}
...
Clean_sau_eez_fes <- sau_eez_fes %>% 
  ...
  mutate(sf_eez_name = ifelse(eez_name =="Greece (without Crete)","Greece", # Vicky
                                                                 ...
                                                                 ifelse(eez_name == "Ecuador (mainland)", "Ecuador",
                                                                        ifelse(eez_name =="Portugal (mainland)","Portugal",
                                                                               ifelse(fishing_entity == "Korea (North)","Korea (North)",
                                                                                      ifelse(fishing_entity == "Guadeloupe  (France)","Guadeloupe (France)",
                                                                                             ifelse(eez_name == "South Africa (Atlantic and Cape)","South Africa (Atlantic Coast)",
                                                                                                    ifelse(eez_name =="Viet Nam","Vietnam",
                                                                                                           ifelse(eez_name =="US Virgin Isl.","US Virgin Islands",
                                                                                                                  ifelse(eez_name == "Spain (mainland, Med and Gulf of Cadiz)", "Spain (Mediterranean and Gulf of Cadiz)",
                                                                                                                         paste(eez_name)
                                                                                                                  )
                                                                                                           )
                                                                                                    )
...
UN_Regions <- read.csv(paste(my_path("D"),"Spatial/UN_Regions.csv",sep=""), header = TRUE) %>% 
  ...
  mutate(
    fishing_entity = ifelse(un_fishing_entity == "Congo","Congo, R. of",
                            ifelse(un_fishing_entity == "Democratic Republic of the Congo	","Congo (ex-Zaire)",paste(fishing_entity))
    )
  )
...
World_sf_df <- as.tibble(World_sf) %>% 
  select(sf_eez_name=eez_name,sf_objectid=OBJECTID,sf_eezid=EEZID)

# Vicky
Clean_sau_eez_fes_new <- Clean_sau_eez_fes %>% 
  left_join(UN_Regions) %>% 
  select(-`n()`) %>% 
  full_join(World_sf_df) %>% 
  select(1:3,4,8,9,everything())

#  Now check tim sau data
SAU_Tim_Data %>% 
  anti_join(Clean_sau_eez_fes_new) %>% 
  group_by(fishing_entity_id) %>% 
  summarise(n())


write_csv(Clean_sau_eez_fes_new,
          "matching_names.csv")
...
# Conuntry Names relation

# matching_country_names <- my_path("D","Spatial","matching_names.csv", read = T) %>% 
#   mutate(eez_name = ifelse(is.na(eez_name),sf_eez_name,eez_name))

# Spatial Data needed to remove duplicates
sau_eez_fes <- fread(paste(my_path("G"),"Spatial/SAU/sau_eez_fes.csv",sep="")) %>% 
  clean_names()
...
Clean_sau_eez_fes <- sau_eez_fes %>% 
  ...
  ungroup() %>% 
  ### Manually Fix names for estimating catch
  mutate(eez_name = ifelse(eez_name =="Greece (without Crete)","Greece", # Vicky
                           ifelse(eez_name =="Greenland (Denmark)","Greenland",
                                  ifelse(eez_name =="United Kingdom (UK)","United Kingdom",
                                         ifelse(fishing_entity =="Italy","Italy",
                                                ifelse(eez_name =="Chile (mainland)","Chile",
...
UN_Regions <- read.csv(paste(my_path("D"),"Spatial/UN_Regions.csv",sep=""), header = TRUE) %>% 
  ...
  mutate(
    fishing_entity = ifelse(fishing_entity == "Congo","Congo, R. of",
                            ifelse(fishing_entity == "Democratic Republic of the Congo	","Congo (ex-Zaire)",paste(fishing_entity))
                            )
  ) %>% 
...
# World_sf %>% filter(str_detect(eez_name, "Cu"))
...
Entity_Area <- World_sf %>% 
  ...
  left_join(Clean_Results_Trans,
            by = "eez_name") %>% 
  group_by(fishing_entity,OBJECTID) %>% 
...
SAU_Tim_Data <- fread(paste(my_path("D"),"Distribution/sau_catch_value_country_taxon_JEPA.csv", sep ="")) %>%
  mutate(value = 1.16 * value)


# Read SAU non-reconstructed catch data from 2006 for comparrisonwith Sumaila
# SAU_Vicky_Data <- fread(paste(my_path("D"),"Distribution/ReportCatch_FEYrEEZTaxon_2010_14.csv", sep ="")) 

SAU_Vicky_Data <- fread(paste(my_path("D"),"Distribution/ReportCatch_FEYrEEZTaxon_2005_09.csv", sep =""))
...
Status_Data <- fread(paste(my_path("R"),"EEZ/Species_Status.csv", sep ="")) %>% 
  clean_names() %>% 
  rename(eez_name=territory) %>% 
  mutate(eez_name = ifelse(eez_name == "Spain (Mediterranean and Gulf of Cadiz)","Spain",eez_name)
         )
...
Clean_Results_Trans <- fread(paste(my_path("R"),"/EEZ/Clean_Results_Trans.csv", sep ="")) %>% 
  mutate(eez_name = ifelse(eez_name == "Cote dIvoire","Côte d'Ivoire",
                           ifelse(eez_name == "Curacao (Netherlands)","Curaçao (Netherlands)",eez_name)
                           ),
         fishing_entity = ifelse(fishing_entity == "Cote dIvoire","Côte d'Ivoire",
                                 ifelse(fishing_entity == "Curacao (Netherlands)","Curaçao (Netherlands)",fishing_entity))
         )
...
### Tnas and billfish according to Pons 2016

tunas <- c("Katsuwonus pelamis",
"Thunnus alalunga",
"Thunnus albacares",
"Thunnus maccoyii",
"Thunnus obesus",
...
# number of eezs
length(unique(sau_eez_fes$eez_name))
# Number of coastal territories
length(unique(sau_eez_fes$fishing_entity))
...
SAU_Sampled_Data <- Clean_sau_eez_fes %>%
  group_by(fishing_entity,fishing_entity_id,eez_name) %>% 
  summarise(n=n()) %>% 
  left_join(SAU_Tim_Data, by = "fishing_entity_id") %>% 
  left_join(Exploited_Species) %>% 
  filter(!is.na(taxon_name)) %>% 
...
x <- Tresholded_T_Data %>% 
  select(taxon_key,eez_name) %>% 
  group_by(taxon_key,eez_name) %>% 
  summarise(n=n()) %>% 
  ungroup() %>% 
  select(-n) %>% 
  left_join(Clean_sau_eez_fes,
  ...
  group_by(Variable) %>%
  summarise(Total_Trans = sum(Value, na.rm=T)) %>% 
  mutate(Per_sampled = Total_Trans/Total_SAU_Vicky*100)
...
```{r comparriosn_wtih_teh}

# Average data for the last 10 years

SAU_Global_Vicky <- SAU_Vicky_Data %>% 
  filter(#eezid != 0,
         year == 2006) %>%
...
Total_SAU_Vicky <- sum(SAU_Global_Vicky$sumcatch,na.rm = T)

Summary_Trans_vicky <- Tresholded_T_Data %>% 
  select(taxon_key,eez_name) %>% 
  group_by(taxon_key,eez_name) %>% 
  summarise(n=n()) %>% 
  ungroup() %>% 
  ...
  left_join(Clean_sau_eez_fes,
            by = "eez_name") %>% 
  left_join(SAU_Global_Vicky,
            by = c("taxon_key","fishing_entity_id")) %>% 
  # filter(eezid != 0) %>%
  group_by() %>%
  summarise(Total_Trans = sum(sumcatch, na.rm=T)) %>% 
  mutate(Per_sampled = Total_Trans/Total_SAU_Vicky*100);Summary_Trans_vicky
...
n_discrete <- Tresholded_T_Data %>% 
  semi_join(Discrete,
            by = "taxon_key") %>% # those that are not only discrete
  group_by(taxon_key) %>% 
  summarise(n()) %>% 
...
Map_Breaks <- c(-1,10,100,1000,2000,12000) 
# Map_Breaks <- c(-1,20,200,2000,1000,20000)

###

Total_Value_EEZ <- sau_eez_fes %>% 
  group_by(fishing_entity_id, fishing_entity) %>% 
  summarise(n=n()) %>% # Get all of the fishing entity and id
  select(-n) %>% 
  left_join(SAU_Global_Data) %>% 
...
Value_Trans_Country <- sau_eez_fes %>% 
  ...
  left_join(Total_Value_EEZ,
            by = c("fishing_entity", "Variable") 
            ) %>% # uncomment for estimating percentage
  # Convert catch to million tonnes
  ungroup() %>% 
  mutate(
    Value = ifelse(Variable == "catch",Value/1000, #million
                         Value),
    Trans_Per = (Value/Total_Value)*100000000 #Return to percentage
  )
...
Catch_Plot <- Catch_Plot +
  ...
  draw_plot_label(
    label = c("Revenue (USD) | Catch (Tons)","Million per Km2"),
                  size = 6,
                  x = c(0.3,0.40),
                  y = c(0.25,0.22)
  )
...
label_data <- Status_Plot_Data %>% 
  ...
  mutate(
    individual = ifelse(individual == "Saint Vincent & the Grenadines","Snt. Vincent & Grenadines", individual)
    )
...
#              Df Sum Sq Mean Sq F value    Pr(>F)    
# observation   2   7088  3544.1  26.305 1.584e-11 ***
# Residuals   447  60226   134.7                      
# ---
# Signif. codes:  0 ‘***’ 0.001 ‘**’ 0.01 ‘*’ 0.05 ‘.’ 0.1 ‘ ’ 1
# 
...
```{r Sub-regional_differences, eval = F}
