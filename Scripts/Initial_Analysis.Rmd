...
936/length(unique(SAU_Global_Data$taxon_key))
...
SAU_FAO_Data <- fread(paste(my_path("D"),"Distribution/LVCatch_byYrEEZFETaxa_2010_14.csv", sep ="")) %>% 
  filter(#eezid != 0,
         year == 2006) %>%
...
SAU_FAO_Data_a <- fread(paste(my_path("D"),"Distribution/LVCatch_byYrEEZFETaxa_2010_14.csv", sep =""))
SAU_FAO_Datab <- fread(paste(my_path("D"),"Distribution/LVCatch_byYrEEZFETaxa_2005_09.csv", sep =""))

# FAO data
total_FAO_catch <- bind_rows(SAU_FAO_Data_a,SAU_FAO_Datab) %>% 
  filter(eezid != 0) %>% 
  group_by(fishing_entity_id,taxon_key,eezid) %>% 
  summarise(mean_c = mean(sumcatch,na.rm=T)) %>% 
  group_by() %>% 
  summarise_at(vars(mean_c),sum)


total_sampled_FO_catch <- bind_rows(SAU_FAO_Data_a,SAU_FAO_Datab) %>% 
  filter(eezid != 0) %>% 
  filter(taxon_key %in% Species_Sampled$TaxonKey) %>% 
  group_by(fishing_entity_id,taxon_key,eezid) %>% 
  summarise(mean_c = mean(sumcatch,na.rm=T)) %>% 
...
Exclusive_Discrete_Status_Data <- Discrete_Exclusive %>% 
  left_join(Status_Data, 
            by =c("eez_name","taxon_key")
  ) %>%
  mutate(status = ifelse(is.na(status),"No Status",status)) %>% 
...
Trans_Dis_Data_Stats <- Discrete_Status_Data %>% 
  filter(observation != "No Category") %>%
  mutate(value = abs(value))

# Discrete Data (just desrete and not trans.not shared)
Discrete_Data_Stats <- Exclusive_Discrete_Status_Data %>% 
  filter(observation != "No Category") %>%
  mutate(value = abs(value))
