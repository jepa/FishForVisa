...
Regional_Area <- World_sf %>% 
  ...
  left_join(Clean_sau_eez_fes) %>% 
  left_join(UN_Regions) %>% 
  filter(fishing_entity_id %in% Clean_sau_eez_fes$fishing_entity_id) %>% 
  group_by(sub_region) %>% 
  summarise(Size_Region = sum(Area_km2,na.rm=T)) %>% 
...
Status_Data <- fread(paste(my_path("R"),"EEZ/Species_Status.csv", sep ="")) %>% 
  ...
  rename(eez_name=territory) %>% 
  mutate(
    eez_name = gsub("Brazil","Brazil (mainland)",eez_name),
    eez_name = gsub("Chile","Chile (mainland)",eez_name),
    eez_name = gsub("Curaçao","Curacao",eez_name),
    eez_name = gsub("Greenland","Greenland (Denmark)",eez_name),
    eez_name = gsub("United Kingdom","United Kingdom (UK)",eez_name),
    eez_name = gsub("US Virgin Islands","US Virgin Isl.",eez_name),
    eez_name = gsub("Vietnam","Viet Nam",eez_name),
    eez_name = ifelse(eez_name == "Cote dIvoire","Côte d'Ivoire",
                      ifelse(eez_name == "Spain", "Spain (mainland, Med and Gulf of Cadiz)",
                             ifelse(eez_name == "Greece","Greece (without Crete)",
                                    ifelse(eez_name == "Ecuador","Ecuador (mainland)",
                                           ifelse(eez_name == "Portugal","Portugal (mainland)",
                                                  ifelse(eez_name == "Italy","Italy (mainland)",
                                                         ifelse(eez_name =="Malaysia (Sarawak)","Sarawak",
                                                                ifelse(eez_name =="Saint Martin (France)","St Martin (France)",
                                                                       ifelse(eez_name =="South Africa (Atlantic Coast)","South Africa (Atlantic and Cape)",
                                                                              ifelse(eez_name =="Saba & Sint Eustaius (Netherlands)","Saba and Sint Eustaius (Netherlands)",
                                                                                     ifelse(eez_name =="Spain (Mediterranean and Gulf of Cadiz)","Spain (mainland, Med and Gulf of Cadiz)",
                                                                                            ifelse(eez_name =="Réunion (France)","Reunion (France)",
                                                                                     eez_name)
                                                                       ))
                                                                       )
                                                         ))))
...
Clean_Results_Trans %>% anti_join(Clean_sau_eez_fes, by = "eez_name") %>% 
  group_by(eez_name) %>% 
  summarise(n())
...
Tresholded_T_Data <- Clean_Results_Trans %>% 
  ...
  left_join(Clean_sau_eez_fes, by = "eez_name")
