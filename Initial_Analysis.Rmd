...
Catch_Country <- SAU_Catch_Data_b %>%
  group_by(eez_name) %>%
  summarise(eez_catch = round(sum(total_catch,na.rm=T),2)) %>%
  left_join(Clean_sau_eez_fes,
            by = "eez_name") %>%
  group_by(fishing_entity) %>%
  summarise(fishing_entity_catch = sum(eez_catch,na.rm=T)/1000000) %>% # Million
...
save_plot(paste(Figures_Path,"100_25_Bins_Value_Region.png",sep=""),
          plot,
          base_height = 8,
          base_width = 8)
