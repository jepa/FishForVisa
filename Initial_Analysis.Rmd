...
EstTransIndex <- function(Spp,Model,EEZ_Size,Neighbors){
  ...
  Tone <- select(Spp_Total_Dist,1,3,4)
  Ttwo <- select(Spp_Total_Dist,Name=2,3,4) # Change column name
  
  # Merge dataframes
  Merged_data <- merge(Tone,Ttwo, by = c("Name","Model")) %>% 
    rowwise() %>% 
    mutate(Total = sum(n_cells_spp.x,n_cells_spp.y)) %>% 
    select(Name,Model,Total) %>% 
    left_join(Spp_Total_Dist,
              by = c("Name","Model")) %>% 
    select(Name,Model,Total)
  ...
  Trans_Spp <- suppressWarnings(SppDist %>%
    ...
    # Commented out because EEZ size might not be the best
      # left_join(EEZ_Size,
    #           by = "Name") %>% 
      # Trying with the distribution of the speices
      left_join(Merged_data,
              by = c("Name","Model")) %>% 
    mutate(Trans_Index = n_cells_spp/Total) %>%  # Include one transboundary species
    group_by(Name,Neighbor_Territory,TaxonKey) %>% 
      summarise(Mean_Trans = mean(Trans_Index))
    )# Mean of all data
