...
UN_Regions <- read.csv(paste(Data_Path,"Spatial_Data/UN_Regions.csv",sep=""), header = TRUE) %>% 
  select(Territory = Country.or.Area,
...
if(Jurisdiction == "EEZ"){
  if(Sys.info()[7] == "carmelia"){
    ...
    # Read Sau Catch Data
    Sau_Catch_Data <- fread(paste(Results_Path,"Speceies_Data/Sau_Catch_Data.csv")) %>% 
    ...
    SAU_Catch_Data <- fread(paste(Data_Path,"Distribution_Data/Sau_Catch_Data.csv",sep = ""), deader = TRUE)
...
Catch_Country <- Sau_Catch_Data %>% 
  ...
  summarise(Total_Catch = round(sum(Mean_Catch,na.rm=T)/1000,2)) %>% 
...
World_sf_si_land$BinsLan <- cut(World_sf_si_land$Total_Catch,breaks = 6)

unique(World_sf_si$Bins)
unique(World_sf_si_land$Bins)

# levels(World_sf_si$Bins)
# levels(World_sf_si_land$Bins_C)

Plot <- ggplot() +
  geom_sf(data = World_sf_si,aes(fill = Bins),colour="white") + # n_trans_spp or Bins
  geom_sf(data = World_sf_si_land,aes(fill = BinsLan),show.legend = FALSE) +
  ...
  #                    na.value = "white"
  # ) +
  scale_fill_manual(values = c("#5BBCD6","#5BBCD6","#F98400","#F98400","#F2AD00","#F2AD00","#00A08A","#00A08A","#FF0000","#FF0000"),
                     na.value = "white"
  ) +
...
### OHI-inspired Circular plot Countries

```{r species_circluar_plot, eval = F, echo =F}
...
Countries_Data <- Clean_Results_Trans %>% 
  ...
  arrange(group,Total_Catch) %>% 
  filter(!is.na(individual)) %>% 
  select(individual,group,value=Total_Catch)
...
ggplot(Countries_Data, 
  ...
  geom_hline(yintercept = Median_trans, colour = "black", alpha=1, size=0.3,linetype="dashed") +
  # Add text showing the value of each line
  annotate("text", 
           x = rep(max(Countries_Data$id),4),
           y = c(20, 40, 60, 80,100),
           label = c("20", "40", "60"),
           color="grey", 
           size=3 , 
  ...
  geom_bar(aes(x=as.factor(id),
           ...
           stat="identity",
           alpha=0.5) +
  ylim(-100,200) + # sets the circle the first value the central the last value how wide
  theme_minimal() +
  theme(
...
ggsave(
    ...
    height = 10,
    units = "in",
    filename = "Territory_circular_over10.tiff",
    path = "/Volumes/DATA/JULIANO_NEYMAR/FishForVisa/Figures/"
  )
