...
This function Read in species distribution from the observation data, ESM models and SAU data
...
# Clean out countries that share with their own borders (e.g. USA gulf of Mx and East coast)
  ...
  Clean_Results <- Auto_Remove_Territories %>% 
    ...
    select(-8,-9) %>% 
    # Fixe coding issues between MAC and Windows
    mutate(
...
```{r Species_Status_fun, eval = T, echo =F}
...
EstSppStatus <- function(Spp,Coord){
  ...
  Data <- GetSppDist(Spp, Model = "SAU_C", Coord = Coord) %>% 
    ...
    filter(!is.na(EEZID),
           Value > 0) %>% 
    group_by(Species,Year,Territory) %>% # Total catch per country
  ...
  # Evaluate species that met the criteria
    ...
    Status_Data <- Data %>% 
      ...
      group_by(Species,Territory,Status) %>% 
    ...
    # If only No status data, then just return that
    if(nrow(No_Status_Data) > 0 & nrow(Status_Data) == 0){
...
Results_Path <- paste(Results_Path,Jurisdiction,"/",Folder,"/",sep="")
...
if(Sys.info()[7] == "carmelia"){ # For Carmelia
  ...
  # List of species
  Spp <- list.files(paste(Results_Path,sep=""))[-829]
...
World_sf_si$Bins <- cut(World_sf_si$n_trans_spp,breaks = 4)
World_sf_si_land$Bins <- cut(World_sf_si_land$Total_USD,breaks = 4)

World_sf_si_land <- World_sf_si_land %>% 
  arrange(Total_USD)

World_sf_si <- World_sf_si %>% 
  arrange(n_trans_spp)


# head(World_sf_si)
# head(World_sf_si_land)
...
Plot <- ggplot() +
  geom_sf(data = World_sf_si_land,aes(fill = Bins), colour = "grey80",size = 0.1) +
  geom_sf(data = World_sf_si,aes(fill = Bins),colour = "grey20", size = 0.1) + # n_trans_spp or Bins
  ...
  # scale_fill_manual(values = c(rev(wes_palette(n = 4, name = "Darjeeling1")),
                     ...
                     # na.value = "white"
  # ) +
  scale_fill_manual("",values = c("#5BBCD6","#5BBCD6","#00A08A","#00A08A","#F2AD00","#F2AD00","#FF0000","#FF0000"),
  na.value = "white"
  ) +
...
Plot_Name <- paste(Jurisdiction,"_100_25_Bins_Value.png", sep = "") 
...
Species_Data <- Tresholded_T_Data %>% 
  ...
  select(individual=scientific_name,
         ...
         group = DemersPelag) %>% 
  arrange(group,value) %>% 
  filter(value > 20) # remove all species under 10 (way too many to plot!)
