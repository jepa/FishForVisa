...
Folder <- "Trans_Results"
Results_Path <- paste(Data_Path,"Results/",Jurisdiction,"/",Folder,sep = "")

if(file.exists(Results_Path) == TRUE){
  Results_Path <- paste(Data_Path,"Results/",Jurisdiction,"/",Folder,sep = "")
}else{
  dir.create(paste(Data_Path,"Results/",Jurisdiction,"/",Folder,sep = ""))
  Results_Path <- paste(Data_Path,"Results/",Jurisdiction,"/",Folder,sep = "")
}
...
print(paste("You are running the model for",nrow(Exploited_Species),"Species at the",Jurisdiction,"level and will be saved at",Folder))
```
...
```{r cleaning_function, eval = T, echo = F}
...
# Clean out countries that share with their own borders (e.g. USA gulf of Mx and East coast)
...
if(Sys.info()[7] == "carmelia"){ # For Carmelia
  ...
  Discrete_Data <- fread(paste(Results_Path,"/Trans_Results/Non_Transboundary_Data.csv",sep="")) %>% 
...
### Non-transboundary
# 
# Partial_Transboundary_Spp <- Clean_Results_Trans %>% 
#   filter(
#          Area_Index >= Min_AI,
#          Area_Index <= Max_AI,
#          Model_Index == MI
#          ) 
# 
# Discrete_Spp_Area_Index <- Clean_Results_Trans %>% 
#   filter(
#     Area_Index >= Max_AI, # Discrete spp only on countryes with > 75%
#     # Area_Index <= Min_AI,
#          Model_Index == MI
#          ) %>% 
#   anti_join(Partial_Transboundary_Spp, # Removes species if it is already transboundary in a country
#             by = c("TaxonKey","Territory")
#   ) %>% 
#   group_by(Territory) %>% 
#   summarise(
#     n_discrete_spp = length(unique(TaxonKey))
    )
  

# Discrete_Spp <- Discrete_Data %>% 
#   group_by(Territory) %>% 
#   summarise(
#     n_discrete_spp = length(unique(TaxonKey))
#     ) %>% 
#   bind_rows(Discrete_Spp_Area_Index) %>% 
#   group_by(Territory) %>% 
#   summarise(n_discrete = sum(n_discrete_spp))

  # Merging eezs by territories (e.g. Mex Atlantic and pacific to Mez)
Country_Discrete_Data <- Discrete_Data %>% 
...
Catch_Country <- Sau_Catch_Data %>% 
  ...
  summarise(Total_Catch = round(sum(Mean_Catch,na.rm=T)/1000000,2)) %>% 
...
World_sf_si_land <- World_land_sf %>%
  st_transform(crs = 4326) %>% # 4326
  # st_transform(crs = "+proj=wintri") %>% # tubed world
...
Plot <- ggplot() +
  geom_sf(data = World_sf_si_land,aes(fill = BinsLan), colour = "grey80") +
  geom_sf(data = World_sf_si,aes(fill = Bins),colour = "grey20") + # n_trans_spp or Bins
  geom_sf(data = Grey_land,aes(fill = BinsLan),fill = "grey80") + # n_trans_spp or Bins
  # scale_fill_gradientn(colours = pal,
  #                      na.value = "white") +
  scale_fill_manual(values = c(rev(wes_palette(n = 4, name = "Darjeeling1")),
                               rev(wes_palette(n = 5, name = "Darjeeling2"))
                               ),
                     na.value = "white"
  ) +
  # scale_fill_manual("",values = c("#5BBCD6","#5BBCD6","#00A08A","#00A08A","#F2AD00","#F2AD00","#F98400","#FF0000","#FF0000","#FF0000"),
  #                    na.value = "white"
  # ) +
...
Plot_Name <- paste(Jurisdiction,"_100_25_Bins_Landb.png", sep = "") 
...
Data <- World_sf_si_land %>% 
  as_tibble() %>% 
  select(Territory, BinsLan)

Data_EEZ <- World_sf_si %>% 
  as_tibble() %>% 
  select(Territory, Bins)
...
Discrete_Data <- Clean_Results_Trans %>% 
  ...
  bind_rows(Country_Discrete_Data) %>%
...
Countries_Data <- Discrete_Data %>% 
  anti_join(Countries_Data,
            by = c("individual","group")) %>% 
  mutate(Total_Discrete = 0) %>% 
  bind_rows(Countries_Data)

Discrete_Data <- Countries_Data %>% 
  anti_join(Discrete_Data,
            by = c("individual","group")) %>% 
  mutate(value = 0) %>% 
  bind_rows(Discrete_Data)
...
Char <- as.factor(Countries_Data$group) # Set groups to factors
...
Plot_Limit <- max(Countries_Data$value,na.rm=T)
Plot_Limit_Low <- -110
Ticks_Color <- "grey50"
...
ggplot(Countries_Data,
  ...
  annotate("text", 
           x = rep(max(Countries_Data$id),length(Seq_axis)),
           y = Seq_axis,
...
### Potential connections Map

```{r connection_map, eval = F, echo = F}
...
head(data)
Clean_Results <- read.csv("~/GitHub/FishForVisa/Temporal_Data/Clean_Results.csv")

Clean_Results <- Clean_Results %>% 
  select(TaxonKey, homename = Country_Territory, travelname = Neighbor_Territory)
...
# DBEM Coordinate system
Coor <- read.csv("~/Documents/UBC/Oceans_Project/Distribution/DBEM/Data/Lon_Lat_DBEM.txt",header = FALSE)
colnames(Coor) <- c("INDEX","Longitude","Latitude")

Index_Code <- EEZIDs_List %>% 
  left_join(EEZ_CellID) %>% 
  rename(Territory = Name) %>% 
  left_join(Coor, by = "INDEX") %>% 
  group_by(Territory) %>% 
  summarise(Latitude = mean(Latitude),
            Longitude = mean(Longitude)
  )

Home_coor <- Index_Code %>% 
  rename(
    homename = Territory,
    homelat = Latitude,
    homelon = Longitude)

Travel_coor <- Index_Code %>% 
  rename(
    travelname = Territory,
    travellat = Latitude,
    travellon = Longitude)
...
earth <- jpeg::readJPEG("BlackMarble_2016_01deg.jpg", native = TRUE)
...
summary <- Data %>%
  group_by(homename,travelname) %>% 
  summarise(n = n()) %>% 
  left_join(Home_coor) %>% 
  left_join(Travel_coor)
...
# Plot
library(ggplot2)
pal <- wesanderson::wes_palette("Zissou1", 1000, type = "continuous")
ggplot() +
  annotation_custom(earth, xmin = -180, xmax = 180, ymin = -90, ymax = 90) +
  geom_line(data=data_ready_plot, aes(x=lon,
                                      y=lat, 
                                      group=group,
                                      colour=n
                                      )
