...
EstNeighbors <- function(Shapefile, Partial = "NA"){
  ...
  Result_table = data.frame(Name = Shapefile$Name[mat[,1]], 
                            Neighbor_Territory = Shapefile$Name[mat[,2]]
  )
  
  # Remove duplicates... eg of course an object intersects with itself...
  Result_table = Result_table[Result_table$Name != Result_table$Neighbor_Territory,]
  
  if(Partial == T){
    # If only partial analysis just returns neighbor list
    return(Result_table)
  ...
  DBEM_Neighbors_Data <- Result_table %>% 
...
```{r EEZ_Neighbords_Control_Pannel, eval = F, echo = F}
...
# Run Function (Ran and saved May 27 2019 at 6pm)
 system.time(
  EEZ_Neighbor_List <- EstNeighbors(World_EEZs, Partial = T)
 )
# 8203.05    0.25 8204.51 (around 2 hours 30 min)

write.csv(EEZ_Neighbor_List,
          paste(Data_Path,"Spatial_Data/EEZ_Neighbor_List.csv", sep =""),
          row.names = F)
...
```{r RFMO_Neighbords_Control_Pannel, eval = F, echo = F}
...
# Run Function (Ran and saved May 27 2019 at 6pm)
 system.time(
  RFMO_Neighbor_List <- EstNeighbors(RFMOs_sf, Partial = T)
 )
# 8203.05    0.25 8204.51 (around 2 hours 30 min)

write.csv(RFMO_Neighbor_List,
          paste(Data_Path,"Spatial_Data/RFMO_Neighbor_List.csv", sep =""),
          row.names = F)
...
TenTwenyfive <- Clean_Results_Trans %>% 
  filter(
         Area_Index >= Min_AI,
         Area_Index <= Max_AI,
         Model_Index == MI,
...
Non_trans_Area_TenTwenyfive <- Clean_Results_Trans %>% 
  filter(
         Area_Index < Min_AI |
         Area_Index > Max_AI,
         Model_Index == MI,
...
# Old code
```{r OLD_funEstNeighbors, eval = F, echo=T}
# Determine which countries are neighbors
