...
# The function
EstTransIndex <- function(Spp,Model,Neighbors){
  
  # Get model data from spps
  SppDist <- GetSppDist(Spp,Model)
  ...
  Trans_Spp <- SppDist %>%
    # filter(INDEX %in% Index_EEZ$INDEX) %>% # For testing
...
# Exploited_Species_List <- c(600004,"600006",600107)
# Exploited_Species_List = 600006
...
```{r Routine_parallel_Windows, eval = F, echo = F}
...
clusterExport( cl, c('Neighbor_List') )

## Step 4: Do the computation
par.output <- parLapply(cl, 1:4,
    function(xx) {
       return( EstTransIndex(xx) + Neighbor_List )
    })
...
system.time(
  Spp_Trans <- bind_rows(
    mclapply(
      Exploited_Species_List[1], FUN=EstTransIndex, Model = "All", Neighbors = Neighbor_List, mc.cores = n_Cores
    )
  )
)
