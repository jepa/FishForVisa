...
# use taxize to get species family name

Species_family_list <- Species_Trans %>% 
  mutate(scientific_name = paste(Genus,Species)) %>% 
  pull(scientific_name) %>% 
  tax_name(Species_list, get = "family", db = 'itis')
...
```{n_species_barplot, eval =F, echo =F}

Species_Trans %>% 
  group_by(Genus,DemPel) %>% 
  summarise(Countries_Genus = sum(n_countries,na.rm=T)) %>% 
  mutate(Bins = cut(Countries_Genus, breaks = 5)) %>% 
  filter(Countries_Genus < 200,
         !is.na(DemPel)) %>%
  ggplot(.,
       aes(
         x = reorder(Genus,Countries_Genus),
         y = Countries_Genus,
