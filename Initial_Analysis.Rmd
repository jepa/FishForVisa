...
Neighbours_Data <- read_excel(paste(Data_Path,"Spatial_Data/EEZ_Neighbours_List.xlsx", sep =""))
...
World_sf_si_land$Bins <- cut(World_sf_si_land$Total_Catch,breaks = 4)
...
Plot <- ggplot() +
  # geom_sf(data = World_sf_si_land,aes(fill = Bins), colour = "grey80") +
  geom_sf(data = World_sf_si,aes(fill = Bins),colour = "grey20") + # n_trans_spp or Bins
  geom_sf(data = Grey_land,aes(fill = Bins),fill = "grey80") + # n_trans_spp or Bins
...
Countries_Data_Plot <- Tresholded_Data %>% 
  group_by(Territory = CT_fishing_entity) %>% 
  summarise(value = length(unique(TaxonKey))) %>% 
