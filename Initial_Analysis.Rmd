...
CreateFigureOne <- function(Variable_Selected){
  ...
  Grey_land <- World_land_sf %>%  
    filter(!NAME_EN %in% World_sf_si_land$NAME_EN)
  ...
  geom_text(data=label_data_catch, 
            aes(x=id,
                y=ifelse(Value_sum == 12,Value_sum,Value_sum+0.1), # Fix names overlap
                label=sub_region,
                hjust=hjust), 
            color="black",
            fontface="bold",
...
Species_Sampled <- fread(paste(Data_Path,"Distribution_Data/Sampled_Species.csv", sep ="")) 
...
Entity_Area <- World_sf %>% 
  as.data.frame() %>% 
  left_join(Clean_Results_Trans,
            by = "eez_name") %>% 
  group_by(fishing_entity) %>% 
  summarise(Size_Entity = sum(Area_km2))
...
# Set global values for catch and revenue (within EEZ with re-constructed catch)
SAU_Global_Data <- Clean_sau_eez_fes %>%
...
Total_SAU_id <- SAU_Global_Data %>% 
  left_join(Exploited_Species) %>% # list of SAU exploited species identified
  filter(!is.na(taxon_name)) %>% # reove those NAs 
  group_by(Variable) %>% 
...
Total_Catch_Spp <- Total_SAU_Sampled$Total_Sampled[1]/Total_SAU_id$Total_id[1] *100 #96.57%

# Percentage species analyzed from the identified species
Total_spp_analyzed <-Total_SAU_Sampled$Sampled_nspp[1]/Total_SAU_id$id_nspp[1]*100 #63.2%


# When we do not remove the groups that are not species level the number sampled drops to ~60%

### So what species are NOT there? 
#  Absten species
# Missing_Species <- SAU_Global_Data %>%
#   filter(!taxon_key %in% Species_Sampled$TaxonKey) %>% 
#   left_join(Exploited_Species) %>% 
...
Summary_Trans <- Clean_sau_eez_fes %>%
  group_by(fishing_entity,fishing_entity_id) %>% 
  summarise(n=n()) %>% 
  left_join(SAU_Sampled_Data) %>% 
  semi_join(Tresholded_T_Data,
            by = c("taxon_key","fishing_entity")) %>% # only counting the catch from transboundary speices (e.g. anchoveta from chile and peru but not from equador)
  group_by(Variable) %>%
...
Discrete <- Clean_Results_Trans %>% 
  ...
  group_by(taxon_key,fishing_entity,eez_name) %>% 
  summarise(n()) %>% 
...
n_discrete <- length(unique(Discrete$taxon_key))

  Summary_Discrete_B <- Discrete_B %>% 
  left_join(SAU_Sampled_Data,
           by = c("taxon_key","eez_name")) %>% 
  group_by(Variable) %>%
...
Discrete_Exclusive <- Clean_Results_Trans %>% 
...
Summary_Discrete_Exclusive <- Discrete %>% 
  left_join(SAU_Sampled_Data) %>% 
  group_by(Variable) %>%
  summarise(Total_Discrete = sum(Value, na.rm=T)) %>% 
  left_join(Total_SAU_Sampled) %>% 
...
Map_Breaks <- c(-1,10,100,1000,5000,10000)

Value_Trans_Country <- sau_eez_fes %>% 
  ...
  group_by(fishing_entity,Variable) %>% # Estimate the amount of dollars of each nation 
  summarise(
    Trans_Entity_n = length(unique(taxon_key)),
    Trans_Total = sum(Value,na.rm=T) #million
  ) %>%
  ungroup() %>%
  filter(!is.na(Variable)) %>% # Momentaneus fix for some reason there are diff. between datasers
  # Match the sacle limits of both value and and catch 
  mutate(
    Trans_Total = ifelse(Variable == "value",Trans_Total/1000000, #million
                         ifelse(Variable == "catch",Trans_Total/1000, #thousand
                                Trans_Total)
                         )
  ) %>% 
  spread(Variable,Trans_Total) %>% 
  left_join(Entity_Area, by = "fishing_entity") %>% 
  # Include the weighted method for supplementals
  mutate_at(
    vars(catch,value),
    funs(weight= ./(Size_Entity/1000000))
  ) %>% 
  gather("Variable","Value",catch:value,catch_weight:value_weight) %>% 
  group_by(fishing_entity,Variable,Value) %>% 
  summarise(
...
Continent_Trans_Value <- Value_Trans_Country %>% 
  ...
  # select(-Size_Entity,-Trans_Entity_n)

## Sub region leven

Subregion_Trans_Value <- Value_Trans_Country %>% 
  left_join(UN_Regions,
            by = "fishing_entity") %>% 
  mutate(sub_region = ifelse(is.na(sub_region), "Other",paste(sub_region))) %>% 
  group_by(sub_region,Variable) %>% 
  summarise_if(is.numeric,c(Value_sum = sum,
                            Value_m = mean,
                            Value_sd = sd),
               na.rm=T) %>%
  # select(-Size_Entity,-Trans_Entity_n) %>% 
...
# Fix names for plot
Subregion_Trans_Value$sub_region <- Subregion_Trans_Value$sub_region %>% 
  gsub("Northern","N.",.) %>% 
  gsub("Southern","S.",.) %>% 
  gsub("Western","W.",.) %>% 
  gsub("Eastern","E.",.)
...
# Variable to plot (value or catch)
Variable_plot <- "catch"
...
Weighted_Data <- Subregion_Trans_Value %>% 
  filter(Variable %in% Weighted_values)
...
Catch_Plot <- Catch_Plot +
  ...
  # Legend
  draw_plot(legend_plot, x = 0.2, y = 0.1, width = 0.59, height = 0.1) +
  # Labeks
  draw_plot_label(label = c("A", "B"),
                  size = 15,
                  x = c(0.1, 0.6),
  ...
  draw_plot_label(
    label = c("Revenue (USD) | Catch (Tones)","Million per Km2"),
                  size = 6,
                  x = c(0.3,0.40),
                  y = c(0.25,0.22)
  )
  ...
  save_plot(
    paste(Figures_Path,"weight_plot.png",sep=""),
    p,
    base_height = 4,
    base_width = 6
  )
...
Top_Rev_Countries <- c("Peru", "Russian Federation", "Chile", "USA")
...
Top_Rev <- sau_eez_fes %>% 
  ...
  group_by(fishing_entity) %>%
  top_n(16,Value) %>%
  select(fishing_entity,Variable,Value,taxon_name) %>% 
  mutate(Top_Value = Value/1000000) %>% # Million of USD
  select(-Value) %>% 
  left_join(Total_Top_Rev,
            by = c("fishing_entity","Variable")
            ) %>% 
  mutate(Ratio = Top_Value/Value*100)
...
Catch_aov <- aov(fishing_entity_catch~sub_region, data = Subregion_Catch_stats)
Catch_aov
...
Total_Trans_Status <- Status_Data_Stats %>% 
  group_by() %>% 
...
Status_Data_Stats %>% 
  ...
  mutate(Sum/Total_Trans_Status*100)
...
bartlett.test(value~observation, Status_Data_Stats)

# Differences between categories of transboundary species

Stat_Cat_kt <- kruskal.test(observation~value, data = Status_Data_Stats)
Stat_Cat_kt

# Post hock test
kruskalmc(Status_Data_Stats$value,Status_Data_Stats$observation)
...
#           Df Sum Sq Mean Sq F value Pr(>F)    
# observation   2  22093   11046   52.88 <2e-16 ***
# Residuals   446  93170     209           
...
# Run anova within discrete speciess

Discrete_Data_Stats <- Discrete_Status_Data %>% 
  filter(observation != "No Category") %>%
  mutate(value = abs(value))

Total_Discrete_Status <- Discrete_Data_Stats %>% 
  group_by() %>% 
  summarise(sum = sum(value)) %>% 
  pull()
...
Discrete_Data_Stats %>% 
  ...
  mutate(Propo_Total = Sum/Total_Discrete_Status*100)
...
# ANOVA of Discrete continents
# Stat_discrete_cont_aov <- aov(value~group, data = Discrete_Data_Stats)
# Stat_discrete_cont_aov
# summary(Stat_discrete_cont_aov)

# Df Sum Sq Mean Sq F value Pr(>F)
# group         5    962   192.4    1.18  0.318
# Residuals   377  61472   163.1 


# -------------------------- #
# Comparrison bwtween Discrete
...
Manova_Differences <- Status_Data_Stats %>% 
...
# observation   2 0.20992   26.151      4    892 < 2.2e-16 ***
# Residuals   446             
summary.aov(mano)
# Response Discrete_value :
#              Df Sum Sq Mean Sq F value    Pr(>F)    
# observation   2   6918  3458.9  25.684 2.769e-11 ***
# Residuals   446  60063   134.7                      
# ---
# 
# 
#  Response Trans_value :
#              Df Sum Sq Mean Sq F value    Pr(>F)    
# observation   2  22093 11046.3  52.878 < 2.2e-16 ***
# Residuals   446  93170   208.9                      
# ---

#### Run non-parametric to make sure (same)

kruskal.test(Trans_value~Discrete_value, data = Manova_Differences)
# Kruskal-Wallis chi-squared = 296.66, df = 43, p-value < 2.2e-16

### Multiple wilcox/t.test

Manova_Differences %>% 
  # group_by(observation) %>% 
  ungroup() %>% 
  select(2,4,5) %>% 
  do(wil = aov(.$Trans_value~.$Discrete_value)) %>% 
  tidy(wil) %>% 
  View()
