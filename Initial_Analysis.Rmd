...
#### For testing 
# Spp <- 604250  # Anchovie Equator, Chile and Peru
...
EstTransIndex <- function(Spp,Model,Neighbours,Coord,Index_Code,Save="Y",Jurisdiction_Type){
  ...
  if(nrow(Spp_Grid) == 0 & Save == "Y"){
    ...
    # IF SPECIES IS DISCRETE AND ONLY APEARS IN ONE REGION
  ...
  n_Territory <- length(Discrete_Territory)
    ...
    To_Print <- paste(Spp, "is only present in",Discrete_Territory)
    print(To_Print)
    paste(Spp, "is only present in",Discrete_Territory)
    }else{
      
      To_Print <- paste(Spp, "is in both (non-neighbouring)",Discrete_Territory[1],Discrete_Territory[2])
      print(To_Print)
    }
...
# In case you want to use old results (ran with older verion of neighbours)
...
if(Sys.info()[7] == "carmelia"){ # For Carmelia
  ...
  # List of species
  Spp <- list.files(paste(Results_Path,"/Trans_Results",sep=""))[1:672]
  ...
  Auto_Remove_Territories <- Results_Trans %>%   
    ...
    select(TaxonKey,
           ...
           FishingRegion=Neighbour, #Neighbour_Territory
           Model_Index,
           Area_Index,
...
ggplot(Countries_Data, 
  ...
  annotate("text", 
           x = rep(max(Countries_Data$id),7),
           y = c(-20,0,20, 40, 60, 80, 100),
           label = c("-20","0","20", "40", "60","80",">100"),
           color="grey", 
           size=3 , 
           angle=0, 
           fontface="bold",
