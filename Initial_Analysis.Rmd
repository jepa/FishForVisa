...
# The function
EstTransIndex <- function(Spp,Model,Neighbors,Coord){
  ...
  # Estimating total grids per country
  Spp_Grid <- Trans_Spp %>% 
  ...
  Tone <- select(Spp_Grid,1,3,4)
  Ttwo <- select(Spp_Grid,Name=2,3,4) # Change column name
  
  # Merge dataframes
  Merged_data <- full_join(Tone,Ttwo, by = c("Name","Model_Index")) %>% 
    rowwise() %>% 
    mutate(Spp_Total = sum(n_cells_spp.x,n_cells_spp.y,na.rm=T)) %>% 
    select(Name,Model_Index,Spp_Total) %>% 
    left_join(Spp_Grid,
              by = c("Name","Model_Index")) %>% 
    ungroup() %>% 
    group_by(Name,Model_Index) %>% 
    summarise(Spp_Total = sum(Spp_Total))
  ...
  Indexes <- Trans_Spp %>% 
    ...
    #____________ ESTIMATING DISTRIBUTION INDEX (TRESHOLD 2)_________ #
    left_join(Merged_data,
              by = c("Name","Model_Index")) %>% 
    mutate(Trans_Index = n_cells_spp/Spp_Total)
