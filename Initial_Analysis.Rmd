...
Catch_Country <- SAU_Catch_Data %>%
  ...
  mutate(fishing_entity = ifelse(fishing_entity == "USA","United States of America",
         ...
         Bins = cut(fishing_entity_catch,breaks = 4),
         Bins_Plot = cut(fishing_entity_catch,breaks = 4,
                         labels = c('super low', 'low','high','super high'))
  ) %>% 
...
Continent_Data_Trans <- Tresholded_T_Data %>% 
  ...
  mutate(id = seq(1,nrow(Continent_Data_Trans)))

# Fix names for plot
Continent_Data_Trans$sub_region <- Continent_Data_Trans$sub_region %>% 
  gsub("Northern","N.",.) %>% 
  gsub("Southern","S.",.) %>% 
  gsub("Western","W.",.) %>% 
  gsub("Eastern","E.",.)
...
number_of_bar <- nrow(Continent_Data_Trans)

label_data_rev <- Continent_Data_Trans %>% 
  arrange(Total_USD) %>% # soo it looks like a nice snail :)
  mutate(
    id = seq(1,nrow(Continent_Data_Trans)),
...
label_data_catch <- Continent_Data_Trans %>% 
...
# Revenue Plot
# Set plot limits
max_lim <- max(Continent_Data_Trans$Total_USD)

# Set color pal
pal <- wes_palette("Zissou1",100,type = "continuous")

Rev_Plot <- ggplot(data = Continent_Data_Trans, 
...
max_lim <- max(Continent_Data_Trans$fishing_entity_catch)

Catch_Plot <- ggplot(data = Continent_Data_Trans, 
