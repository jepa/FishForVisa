...
head(Results_Data)
unique(Results_Data$Model_Index)
length(unique(Results_Data$TaxonKey)) #672 spp
...
write.csv(SubsetData,
          "SubsetData.csv",
          row.names = F) # Using the US east coast
```

## Number of transboundary species per country

```{Number_Tran_country, eval =F, echo =F}
# TI of 10% and MI > 50
...
TenTwenyfive <- Clean_Results %>% 
  ...
  group_by(Name=Country_Territory,Model_Index) %>% 
  summarise(
    n_trans = length(unique(TaxonKey)),
    n_countries = length(unique(Neighbor_Territory)),
