...
# Set paths depending on machine Beas or Hall1000
...
EstTransIndex <- function(Spp,Model,Neighbors,Coord,Save="Y"){
  ...
  Spp_Grid <- Trans_Spp %>% 
    group_by(TaxonKey,Model_Index,EEZID) %>% 
  ...
  Country_Territory_T <- Spp_Grid %>% 
    ungroup() %>% 
    select(Model_Index,TaxonKey,Name=Country_Territory,n_spp_eez)
  
  Neighbor_Territory_T <- Spp_Grid %>% 
    ungroup() %>% 
    select(Model_Index,TaxonKey,n_spp_eez,Name=Neighbor_Territory, Country_Territory)

  # Merge dataframes to get totals per neighbords
  Area_Index_D <- full_join(Country_Territory_T,
                           Neighbor_Territory_T, by = c("Model_Index","Name","TaxonKey")) %>% 
    rowwise() %>%
...
system.time(
  Spp_Trans <- bind_rows(
   ...
   # user  system elapsed (For three spp in Hall1000)
 # 13.430   5.126  35.059 
### Save Test 
...
Results_Data <- GetTransResults(Spp)
...
This might be fixed with Tayler's data of EEZs to ISOs
...
```{Number_Tran_country, eval =T, echo =F}
# TI of 10% and MI > 50

TenFifty <- Non_duplicate_Data %>% 
  filter(Model_Index >= 50#,
         # Trans_Index >= 0.10
         ) %>% 
  group_by(Territory1=fishing_entity,Model_Index) %>% 
  summarise(
    n_trans = length(unique(TaxonKey)),
    n_countries = length(unique(Neighbor_Territory))
    ) %>% 
...
# The path
path_world <- paste("~/Documents/UBC/Oceans_Project/Distribution/Data/Spatial/World_EEZ_v9_20161021",sep="")
# #The File
fnam_world <- "eez_boundaries.shp"
# #Load it!
# 
World_EEZs <- st_read(dsn = path_world,
                      layer =file_path_sans_ext(fnam_world))
...
Plot <- ggplot(World_EEZs_simple) +
  geom_sf(aes(fill = n_Trans_Spp)) +
  scale_fill_gradientn(colours = pal) +
  ggtheme_map()

ggsave("MI_100_TI_all_plot.png",
       plot = Plot,
       width = 16,
       height = 8,
       units = "in",
...
# Salvador <- Results_Data %>% 
#   filter(Name == "El Salvador",
#          Model_Index >= 50)
# 
# Guat_Pac <- Results_Data %>% 
#   filter(Name == "Guatemala (Pacific)",
#          Model_Index >= 50)


  Area_Index_D <- Merged_data %>% 
    filter(Area_Index > 0.10,
           Area_Index < 0.90)

Spp_Results <- Results_Data %>% 
  filter(Model_Index >= 50#,
         # Trans_Index >= 0.10
         ) %>% 
  group_by(TaxonKey,Model_Index) %>% 
  summarise(n_Trans_Spp = n()/2) 


ggplot(Spp_Results) +
  geom_bar(
    aes(
      x = as.character(TaxonKey),
      y = n_Trans_Spp
    ),
    stat = "identity"
  ) +
...
```{r, SOM_Trans_Results, eval = F, echo = F}

Clean_Results <- read.csv("~/GitHub/FishForVisa/Temporal_Data/Clean_Results.csv") %>% 
  filter(!is.na(Country_Territory))

sau_eez_fes <- read.csv("~/Desktop/sau_eez_fes.csv")
  
EEZID_D <- readxl::read_excel("~/Documents/UBC/Oceans_Project/Distribution/By Cell/Cell_Data/Updated_EEZList_17June2016.xlsx")


# Trying to fix EEZ problem... 

sau_eez_fes <-sau_eez_fes %>% 
rename(EEZID = eez_id)
...
Neighbord_Territory_T <- Clean_Results %>% 
  ...
  summarise() %>% 
  left_join(eez_list_n,
            by = "Neighbor_Territory") %>% 
  left_join(sau_eez_fes,
            by = "EEZID") %>% 
  select(Neighbor_Territory,neighbor_entity = fishing_entity)

Non_duplicate_Data <- xx %>%   
  left_join(Neighbord_Territory_T,
              by ="Neighbor_Territory") %>% 
  mutate(
    fishing_entity = ifelse(is.na(fishing_entity),paste(Country_Territory),paste(fishing_entity)),
    Same_Nation = ifelse(fishing_entity==neighbor_entity,paste("Yes"),"No")
  ) %>% 
  filter(Same_Nation == "No")

write.csv(Non_duplicate_Data,
          "New_Clean_Results.csv")
