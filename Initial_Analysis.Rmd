...
label_data_rev <- Subregion_Data_Trans %>% 
  arrange(Total_USD) %>% # soo it looks like a nice snail :)
  mutate(
...
label_data_catch <- Subregion_Data_Trans %>% 
  arrange(fishing_entity_catch) %>% # soo it looks like a nice snail :)
  mutate(
    id = seq(1,nrow(Subregion_Data_Trans)),
    angle = 90 -360 * (id-0.5)/number_of_bar,
    hjust = ifelse(angle < -90,1,0),
...
# Labels_Trans <- c(paste(unique(Transboundary_Spp$Bins)),"NA")
# Labels_Rev <- unique(Revenue_Country$Bins)
...
# Revenue Plot
# Set plot limits
max_lim <- max(Subregion_Data_Trans$Total_USD)

# Set color pal
pal <- wes_palette("Zissou1",100,type = "continuous")

Rev_Plot <- ggplot(data = Subregion_Data_Trans, 
                   aes(x=reorder(id,Total_USD),
                       y=Total_USD, 
                       fill=Total_USD
                   )
) +    
  ylim(-1500, max_lim+10)+
  geom_bar(stat="identity") +
  theme_minimal() +
  theme(
    legend.position = c(0.74,0.60),
    axis.text = element_blank(),
    axis.title = element_blank(),
...
max_lim <- max(Subregion_Data_Trans$fishing_entity_catch)

Catch_Plot <- ggplot(data = Subregion_Data_Trans, 
                     aes(x=reorder(id,fishing_entity_catch),
                         y=fishing_entity_catch, 
                         fill=fishing_entity_catch
                     )
) +    
  ylim(-1.5, max_lim+1) +
  geom_bar(stat="identity") +
...
plot <- ggdraw() +
  ...
  # Revenue circular
  draw_plot(Rev_Plot, x = 0.1, y = 0.07, width = 0.5, height = 0.4) +
  # Catch circular
  draw_plot(Catch_Plot, x = 0.45, y = 0.07, width = 0.5, height = 0.4) +
...
PeRu <- c("Peru", "Russian Federation","Russia", "Chile", "United States of America","USA")

Total_PeRu <- Revenue_Country %>% 
  filter(fishing_entity %in% PeRu) %>% 
  select(1:2) %>% 
  mutate(fishing_entity = ifelse(fishing_entity == "Russia","Russian Federation",
                                 ifelse(fishing_entity == "United States of America","USA",fishing_entity))
  )
...
Plot_Name <- paste("RFMO_100_25_Bins.png", sep = "") 
...
Status_Data_Plot <- Tresholded_T_Data %>% 
  ...
  mutate(value = ifelse(Total_trans > 100,100,Total_trans)) %>% 
  filter(!is.na(individual),
         !str_detect(individual, ".\\(.") # remove associated territories
         )%>% 
...
# data$id <- as.integer(data$individual,na.rm=T)
...
data %>% 
  ...
  geom_segment(data=grid_data, aes(x = end, y = 50, xend = start, yend = 50), linetype = "dashed", colour = "grey", alpha=1, size=0.3, inherit.aes = FALSE ) +
...
### What about... Catch Status...


taxon_status <- Tresholded_T_Data %>% 
  group_by(taxon_key) %>% 
  summarise(
    n_countries = length(unique(eez_name))
  ) %>% 
...
modcs <- lm(n_countries~status, taxon_status)
modcs
summary(modcs)
