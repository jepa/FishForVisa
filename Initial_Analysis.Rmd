...
EstSppStatus <- function(Spp,Coord){
  ...
  Data <- GetSppDist(Spp, Model = "SAU_C", Coord = Coord) %>% 
    ...
    filter(!is.na(EEZID),
           Value > 0) %>% 
    group_by(TaxonKey=Species,Year,Territory) %>% # Total catch per country
    summarise(Total_Catch = sum(Value,na.rm=T)) %>% 
    group_by(TaxonKey,Territory,Year) %>% 
...
```{r spp_status_data eval = T, echo = F}
...
```{r species_status_routine eval = T, echo = F}
...
write.csv(Species_Status,
          paste(Data_Path,"Distribution_Data/Species_Status.csv",sep=""),
                row.names = F)
...
Revenue_Country <- sau_eez_fes %>% 
  ...
  group_by(Territory) %>% 
  summarise(Total_USD = (sum(Mean_Value,na.rm=T))/1000000) %>% 
