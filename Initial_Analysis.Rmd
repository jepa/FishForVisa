...
# The function
EstTransIndex <- function(Spp,Model,Neighbours,Coord,Index_Code,Save="Y",Jurisdiction_Type){
...
## Routine for estimating transboundary species
...
if(Sys.info()[7] == "carmelia"){
  ...
  Auto_Remove_Territories <- Results_Trans %>%   
    ...
    select(TaxonKey,
           ...
           FishingRegion=Neighbour_Territory,
           Model_Index,
           Area_Index,
    ...
    left_join(Clean_sau_eez_fes,
              by = "FishingRegion") %>% 
    select(1,2, 
           Neighbour_Territory = FishingRegion,
    ...
    mutate(
      ...
      NT_fishing_entity = ifelse(is.na(NT_fishing_entity),paste(Neighbour_Territory),paste(NT_fishing_entity)),
  ...
  Territories_to_Remove <- Results_Trans %>% 
    filter(
      Country_Territory %in% Islands_to_Remove |
        Neighbour_Territory %in% Islands_to_Remove |
        Country_Territory == "Balearic Island (Spain)" & Neighbour_Territory == "Spain (Mediterranean and Gulf of Cadiz)" |
        Country_Territory == "British Virgin Isl. (UK)" & Neighbour_Territory == "Anguilla" |
        Neighbour_Territory == "British Virgin Isl. (UK)" & Country_Territory == "Anguilla" |
        Country_Territory == "Corsica (France)" & Neighbour_Territory == "France (Mediterranean)" |
        Neighbour_Territory == "Corsica (France)" & Country_Territory == "France (Mediterranean)" |
        Country_Territory == "Crete (Greece)" & Neighbour_Territory == "Grece" |
        Neighbour_Territory == "Crete (Greece)" & Country_Territory == "Grece" |
        Country_Territory == "Cyprus (North)" & Neighbour_Territory == "Cyprus (South)" |
        Neighbour_Territory == "Cyprus (North)" & Country_Territory == "Cyprus (South)" |
        Country_Territory == "Channel Isl. (UK)" & Neighbour_Territory == "United Kingdom" |
        Neighbour_Territory == "Channel Isl. (UK)" & Country_Territory == "United Kingdom" |
        Country_Territory == "Glorieuse Islands (France)" & Neighbour_Territory == "Mayotte (France)" |
        Neighbour_Territory == "Glorieuse Islands (France)" & Country_Territory == "Mayotte (France)" |
        Country_Territory == "Guam (USA)" & Neighbour_Territory == "Northern Marianas (USA)" |
        Neighbour_Territory == "Guam (USA)" & Country_Territory == "Northern Marianas (USA)" |
        Country_Territory == "Italy" & Neighbour_Territory == "Sardinia (Italy)" |
        Neighbour_Territory == "Italy" & Country_Territory == "Sardinia (Italy)" |
        Country_Territory == "Italy" & Neighbour_Territory == "Sicily (Italy)" |
        Neighbour_Territory == "Italy" & Country_Territory == "Sicily (Italy)" |
        Country_Territory == "Japan (Daito Islands)" & Neighbour_Territory == "Japan (main islands)"|
        Neighbour_Territory == "Japan (Daito Islands)" & Country_Territory == "Japan (main islands)"|
        Country_Territory == "Japan (Daito Islands)" & Neighbour_Territory == "Japan (Ogasawara Islands)"|
        Neighbour_Territory == "Japan (Daito Islands)" & Country_Territory == "Japan (Ogasawara Islands)"|
        Country_Territory == "Norway" & Neighbour_Territory == "Svalbard Isl. (Norway)" |
        Neighbour_Territory == "Norway" & Country_Territory == "Svalbard Isl. (Norway)" |
        Country_Territory == "Sicily (Italy)" & Neighbour_Territory == "Sardinia (Italy)" |
        Neighbour_Territory == "Sicily (Italy)" & Country_Territory == "Sardinia (Italy)" |
        Country_Territory == "South Africa (Atlantic Coast)" & Neighbour_Territory == "South Africa (Indian Ocean Coast)" |
        Neighbour_Territory == "South Africa (Atlantic Coast)" & Country_Territory == "South Africa (Indian Ocean Coast)" |
        Country_Territory == "USA (Alaska, Arctic)" & Neighbour_Territory == "USA (Alaska, Subarctic)" |
        Neighbour_Territory == "USA (Alaska, Arctic)" & Country_Territory == "USA (Alaska, Subarctic)"
    ) %>%
    group_by(Country_Territory,Neighbour_Territory) %>% 
    summarise(n())
  
  # Cleaned results 
  Clean_Results_Trans <- Auto_Remove_Territories %>% 
    anti_join(Territories_to_Remove,
              by = c("Country_Territory","Neighbour_Territory")
    ) %>% 
...
Discrete_Spp <- Clean_Results_Trans %>% 
  ...
  group_by(Name=Country_Territory) %>% 
  summarise(
    n_no_trans = length(unique(TaxonKey)),
    n_countries = length(unique(Neighbour_Territory))
  )
...
## Validate Results using RFMOs

```{Validation_RFMOs}

#### Load RFMO data provided by Vicky (SAU)

RFMOs_sf <- fread(paste(Data_Path,"Spatial_Data/SAU_RFMO/SAU_RFMO_Sept_2015.shp",sep=""))
...
```{n_species_country, eval =F, echo =F}
...
Species_Trans <- Clean_Results_Trans %>% 
  ...
  group_by(TaxonKey) %>% 
  summarise(
    n_countries = length(unique(Country_Territory))
  ) %>% 
  left_join(Exploited_Species) %>%
