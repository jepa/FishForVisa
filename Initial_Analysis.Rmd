...
if(Sys.info()[7] == "jepa88"){
  ...
  Path <- paste(Data_Path,"Results/All_Models/",sep = "")
  }
...
EstTransIndex <- function(Spp,Model,Neighbors,Coord,Save="Y"){
  ...
  Trans_Spp <- SppDist %>%
    ...
    filter(Value > 0) %>%  # Remove latter, just keeps places with 1
    # Commented out to produce a map for each dataset
    # group_by(TaxonKey,
    #          INDEX
    # ) %>%
  ...
  Model_Index_D <- Trans_Spp %>% 
    group_by(Name,
             ...
             Model # Remove after producing models x datasets
    ) %>% 
  ...
  Area_Index_D <- full_join(Country_Territory_T,
                           Neighbor_Territory_T, by = c(#"Model_Index",
                             "Name","TaxonKey","Model")) %>% # Remove Model abd undomment Model_Index after producing Models x datasets
    rowwise() %>%
    ...
    # select(1:3,6,8) # Commented out for all models analysis
    select(1:4,6,8)
  ...
  full_join(Area_Index_D,
            by = c("TaxonKey","Country_Territory",
                   #"Model_Index"
                   "Model") # Remove Model abd undomment Model_Index after producing Models x datasets
  ) %>% 
...
# For Function
Exploited_Species_List <- Exploited_Species$TaxonKey[1:3]
...
write.csv(tibble(Non_trans = "NA"), paste(Results_Path,"Non_Transboundary_Data.csv",sep=""), row.names = F)
...
# Data needed to remove duplicates
...
# Manually deal with NAs... Ughhhhh
Results_NA <- Clean_Results %>% 
  filter(is.na(Same_Nation)) %>% 
