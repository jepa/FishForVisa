...
GetSppDist=function(Spp,Model){
  ...
  if(Model == "All"){
    ...
    sppdist$INDEX <- INDEX
    colnames(sppdist) <- c("SAU","Occ","ENM","INDEX")
...
# head(getSppDist(600004,"SAU")) # Works
...
# The function
EstTransIndex <- function(SppDist,EEZ_Size,Neighbors){
  ...
  Trans_Spp <- SppDist %>%
    ...
    summarise(n_spp = n()) %>% # The number of cells present within each country's EEZ
    left_join(EEZ_Size,
              by = "Name") %>% 
    mutate(Trans_Index = n_spp/n_grid) %>%  # Include one transboundary species
    select(EEZID,everything())  # For the sake of my brain
    
  
  # Result 2. List of Countries that share the Species ####
  
  Countries_with_Specie <- Trans_Spp %>% 
    filter(Trans_Index == 1) %>% 
...
SAU_Data <- read.csv("/Volumes/DATA/JULIANO_NEYMAR/FishForVisa/Distribution_Data/SAU_Catch_Data_Test/SAU_Catch_Data_Test.csv")
...
# TakonKey <- Spps$TaxonKey[1:2]
TakonKey <- c("600004","600006",600107)
# Spp = 600988
...
NOTA: Explorando resultados y hay que arrelgar la cuestion de los tres modelos


```{Routine, eval = T, echo = F}
...
# Step 2; GetSppDist
# Spp_Dist <- GetSppDist(TakonKey[1],"All")
# head(Spp_Dist)

# Multiple TaxonKeys
Spp_Dist <- NULL
Spp_Dist <- lapply(TaxonKey, FUN = GetSppDist, "All")

Spp_Dist <- Spp_Dist[!sapply(Spp_Dist, is.null)]
# Step 3; EstTransIndex
# Spp_Trans <- EstTransIndex(Spp_Dist,EEZ_Size,Neighbor_List)
# Spp_Trans

# Multiple TaxonKeys
Spp_Trans <- NULL
Spp_Trans <- bind_rows(lapply(Spp_Dist, FUN=EstTransIndex, EEZ_Size = EEZ_Size, Neighbors = Neighbor_List))

Spp_Trans %>% 
  mutate(Uncert = ifelse(Trans_Index >= 0.10 & Trans_Index <= 0.25,"10 %",
                         ifelse(Trans_Index >= 0.25 & Trans_Index <= 0.50,"25 %",
                                ifelse(Trans_Index >= 0.50,">50 %","No Trans")))
         ) %>% 
