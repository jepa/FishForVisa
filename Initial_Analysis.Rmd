...
EEZ_Catch %>% 
  filter(
    ...
    year > 2005) %>% 
  group_by(year) %>% 
  summarise(sum_year = sum(tonnes),
            sum_year_value = sum(landed_value)) %>% 
  group_by() %>% 
  summarise(mean(sum_year),
            mean(sum_year_value)
  )


Total_Catch_Spp <- 71842644/75202371
...
Total_Trans_Rev <- SAU_Revenue_Data %>% 
...
Transboundary_Spp <- Tresholded_T_Data %>% 
  ...
  mutate(
    ...
    Weight_Spp = n_trans_spp/Size_EEZ 
...
```{r Territory_Map_Round_Weighted, eval =F, echo =F}
