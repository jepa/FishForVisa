...
Clean_sau_eez_fes <- sau_eez_fes %>% 
  ...
  group_by(eez_name,fishing_entity) %>% 
  summarise(n()) %>% 
  ungroup() %>% 
...
RFMO_Species <- read.csv(paste(Data_Path,"Spatial_Data/RFMO/clean_rfmos.csv", sep ="")) %>% 
  clean_names() %>% 
  left_join(Exploited_Species,
            by = c("taxon_name"))
...
Clean_Results_Trans <- Clean_Results_Trans %>% 
  filter(taxon_key %in% RFMO_Species$taxon_key)
...
Catch_Country <- SAU_Catch_Data %>%
  ...
  summarise(fishing_entity_catch = sum(eez_catch,na.rm=T)/1000000) %>% # Million tones
  mutate(fishing_entity = ifelse(fishing_entity == "USA","United States of America",
         ...
         Bins = cut(fishing_entity_catch,breaks = 5),
         Bins_Plot = cut(fishing_entity_catch,breaks = 5,
                         labels = c('super low', 'low','median','high','super high'))
  ) %>% 
...
```{r Territory_Map_Round, eval =F, echo =F}
...
lenght(unique(Clean_Results_Transboundary$taxon_key))
...
Transboundary_Spp %>% 
  ...
  geom_bar(
    aes(
      x = reorder(eez_name,-n_trans_spp),
      y = n_trans_spp
    ),
    stat = "identity"
  ) +
...
Plot <- ggplot() +
  ...
  geom_sf(data = World_sf_si,
          aes(
            fill =  n_trans_spp#,
            # colour = reorder(Bins_Plot,n_trans_spp)
          ), 
          size = 0.1
          ) + # n_trans_spp or Bins
  ...
  # na.value = "grey"
  # ) +
  # scale_colour_manual("",values = c("#5BBCD6",#"#5BBCD6", # blue
  #                                   "#00A08A",#"#00A08A", #green
  #                                   "#F2AD00",#"#F2AD00", # Yellow
  #                                   "#F98400",#"#F98400", #Orange
  #                                   "#FF0000"#,"#FF0000" #Red
...
Plot_Name <- paste("RFMO_100_25.png", sep = "") 
...
Macrell <- Tresholded_T_Data %>% 
  filter(taxon_key == 600118,
         eez_name %in% Conflict_nations
  )
...
Spp_circular <- ggplot(Species_Data, 
  ...
  theme(
    ...
    axis.title = element_blank(),
    panel.grid = element_blank(),
    # plot.margin = unit(c(-3.5, -3.5, -4, -4), "cm") #top, right, bottom, left
    plot.margin = unit(rep(-3.5,4),"cm")
  ) +
  coord_polar() + 
