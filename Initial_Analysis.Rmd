...
EstSppStatus <- function(Spp,Coord,Save_Path = "NA", Per_Taxa = F){
  ...
  Status_Data <- Data %>% 
    ...
    group_by(TaxonKey,Territory) %>% 
    top_n(1,RMean) %>% 
    group_by(TaxonKey,Territory,RMean) %>%  # Solves for years with the same catch data (e.g. Cayman Isl all years)
    ...
    group_by(TaxonKey,Y_Max_Peak,Territory,Max_Peak,RMean) %>%  # Solves for years with the same catch data (e.g. Cayman Isl 2013-2014)
    summarise(Year = min(Year)) %>% 
  ...
  if(Per_Taxa == T){
    ...
    Name <- paste(Save_Path,"Stock_Status/",Spp,"_Status.csv",sep="")
...
Total_Trans_Rev <- SAU_Revenue_Data %>% 
  filter(TaxonKey %in% Total_trans$TaxonKey) %>% 
  group_by() %>% 
