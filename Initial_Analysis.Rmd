...
CleanResults <- function(Data){
  ...
  Clean_Results$Territory <- Clean_Results$Territory %>% 
    gsub("Cura<e7>ao (Netherlands)","Curaçao (Netherlands)",.) %>% 
    gsub("C<f4>te d'Ivoire","Côte d'Ivoire",.) %>% 
    gsub("R<e9>union (France)","Réunion",.) 
  
  Clean_Results$Neighbour <- Clean_Results$Neighbour %>% 
    gsub("Cura<e7>ao (Netherlands)","Curaçao (Netherlands)",.) %>% 
    gsub("C<f4>te d'Ivoire","Côte d'Ivoire",.) %>% 
    gsub("R<e9>union (France)","Réunion",.) 
  
  Clean_Results$CT_fishing_entity <- Clean_Results$CT_fishing_entity %>% 
    gsub("Cura<e7>ao (Netherlands)","Curaçao (Netherlands)",.) %>% 
    gsub("C<f4>te d'Ivoire","Côte d'Ivoire",.) %>% 
    gsub("R<e9>union (France)","Réunion",.) 
...
Spp_Clasification <- fread(paste(Data_Path,"Distribution_Data/Species/SppTaxonName.csv",sep="")) %>% 
  clean_names()
...
Clean_sau_eez_fes <- sau_eez_fes %>% 
  gather("Type","fishing_region",2,4) %>% 
  left_join(sau_eez_fes,
            by = ("fishing_entity_id")
  ) %>% 
  group_by(eez_name,fishing_entity,fishing_entity_id) %>% 
  summarise(n()) %>% 
  ungroup() %>% 
  ### Manually Fix names for estimating catch
  mutate(eez_name = ifelse(eez_name =="Greece (without Crete)","Greece",
                           ifelse(eez_name =="Greenland (Denmark)","Greenland",
                                  ifelse(eez_name =="United Kingdom (UK)","United Kingdom",
                                         ifelse(fishing_entity =="Italy","Italy",
                                                ifelse(eez_name =="Chile (mainland)","Chile",
                                                       ifelse(fishing_entity =="Brazil","Brazil",
                                                              ifelse(eez_name == "Ecuador (mainland)", "Ecuador",
                                                                     ifelse(eez_name =="Portugal (mainland)","Portugal",
                                                                            ifelse(fishing_entity == "Korea (North)","Korea (North)",
                                                                                   paste(eez_name))
                                                                     )
                                                              )
                                                       )))))),
         fishing_entity = ifelse(fishing_entity == "Viet Nam","Vietnam",fishing_entity
         )
  )
...
UN_Regions <- read.csv(paste(Data_Path,"Spatial_Data/UN_Regions.csv",sep=""), header = TRUE) %>% 
  clean_names() %>% 
  group_by(fishing_entity = country_or_area,
  ...
  mutate(
    fishing_entity = gsub("\\s*\\([^\\)]+\\)","",fishing_entity),
    ...
    # fishing_entity = gsub("Puerto Rico (USA)", "Puerto Rico",fishing_entity),
    fishing_entity = gsub("Taiwan", "Taiwan (China)",fishing_entity),
    fishing_entity = gsub("Côte d’Ivoire", "Côte d'Ivoire",fishing_entity),
    # fishing_entity = gsub("Russian Federation", "Russia",fishing_entity)
    fishing_entity = gsub("Viet Nam", "Vietnam",fishing_entity),
...
World_land_sf <- st_read(dsn = path_world_land,
  ...
  rename(fishing_entity = NAME) %>% 
  mutate(
    fishing_entity = gsub("Russia", "Russian Federation",fishing_entity),
    fishing_entity = gsub( "United States of America", "USA",fishing_entity),
    fishing_entity = gsub("Solomon Islands", "Solomon Isl.",fishing_entity),
    fishing_entity = gsub("Marshall Islands", "Solomon Isl.",fishing_entity),
    fishing_entity = gsub("Dem. Rep. Congo", "Congo, R. of",fishing_entity),
    fishing_entity = ifelse(fishing_entity=="Congo", "Congo (ex-Zaire)",paste(fishing_entity))
  )

# View(unique(World_land_sf$fishing_entity))
...
# View(unique(World_sf$eez_name))


#### Determine Regions ezs areas
...
# Read Sau Catch Data
SAU_Catch_Data <- fread(paste(Data_Path,"Distribution_Data/sau_catch_country_taxon.csv", sep =""))

# Read Sau Value Data (Thank you Tim!)
SAU_Tim_Data <- fread(paste(Data_Path,"Distribution_Data/sau_catch_value_country_taxon_JEPA.csv", sep ="")) 

SAU_Sampled_Data <- Clean_sau_eez_fes %>%
  group_by(fishing_entity,fishing_entity_id) %>% 
  summarise(n=n()) %>% 
  left_join(SAU_Tim_Data, by = "fishing_entity_id") %>% 
  clean_names() %>% 
...
Status_Data <- fread(paste(Results_Path,"Species_Status.csv", sep ="")) %>% 
  clean_names() %>% 
  rename(eez_name=territory)
...
# Read Results
Clean_Results_Trans <- GetTransResults(Spp) %>% 
  CleanResults(.) %>% 
  clean_names() %>% 
  rename(eez_name = territory,
         fishing_entity = ct_fishing_entity,
         eez_neighbour = neighbour) %>% 
  mutate(fishing_entity = ifelse(eez_name == "Spain (Mediterranean and Gulf of Cadiz)","Spain",paste(fishing_entity)))


# Read Results For Carmelia

# Clean_Results_Trans <- fread(paste(Data_Path,"Results/EEZ/Clean_Results_Trans.csv", sep =""))

# Vulnerabillity Data from William
# cc_vul <- fread(paste(Data_Path,"Species_Data/vulspp310817.csv", sep ="")) %>% 
#   rename(taxon_key = V1,
#          ccvul = V2)

# fish_vul <- fread(paste(Data_Path,"Species_Data/FishSenIndex.csv", sep ="")) %>% 
# clean_names()
...
# List of species
Spp_rfmo <- list.files("/Volumes/DATA/JULIANO_NEYMAR/FishForVisa/Results/RFMO/Trans_Results/")

# List of RMOs
rfmo_list <- read.csv("/Volumes/DATA/JULIANO_NEYMAR/FishForVisa/Spatial_Data/RFMO/clean_rfmos.csv") %>% 
  clean_names() %>% 
  left_join(Exploited_Species) %>% 
  filter(!rfmo_name %in% c("IPHC","PSC","NASCO")) # remove these

# Load and clean transboundary results
Results_rfmo <- GetTransResults(Spp_rfmo) %>% 
  clean_names() %>% 
  rename(rfmo_name = territory) %>% 
  # left_join(rfmo_list,
  #           by =c("rfmo_name","taxon_key"))
  filter(
    taxon_key %in% rfmo_list$taxon_key, # just the taxa managed by the rfmos
    !rfmo_name %in% c("IPHC","PSC","NASCO"), # remove these
    !neighbour %in% c("IPHC","PSC","NASCO") # remove these
  )
...
length(unique(Tresholded_T_Data$taxon_key))
...
# Makes sense?
# nrow(Tresholded_T_Data) + nrow(Discrete) == nrow(Clean_Results_Trans) # TRUE

#Number of discrete species #170

n_discrete <- Clean_Results_Trans %>% 
  filter(! taxon_key %in% Tresholded_T_Data$taxon_key) %>% 
  group_by() %>% 
  summarize(length(unique(taxon_key))) %>% 
  pull() 
...
Total_Catch_Spp <- SAU_Sampled_Data/SAU_Total_EEZ_Data

# -------------------------- #
# Number of Transboundary Species
# -------------------------- #

Total_trans <- Clean_Results_Trans %>% 
  filter(
    area_index >= Min_AI,
    area_index <= Max_AI,
    model_index >= MI
...
Total_Trans_Rev <- SAU_Sampled_Data %>% 
  semi_join(Tresholded_T_Data,
            by = c("taxon_key"))
  
  group_by() %>% 
  summarise(Total =sum(mean_value)/1000000)# total revenue within EEZs of the world from sau.org


Per_Trans_Rev <- SAU_Revenue_Data %>% 
  filter(taxon_key %in% Total_trans$taxon_key) %>% 
  group_by() %>% 
  summarise(Total =sum(mean_value)/1000000) %>%  # million
  pull()/160000*100 
...
Total_Trans_Catch <- SAU_Catch_Data %>% 
  filter(taxon_key %in% Total_trans$taxon_key) %>% 
  group_by() %>% 
  summarise(Total =sum(total_catch)/1000000) %>%  # total revenue within EEZs of the world from sau.org
  pull()

Global_Catch <- 80 # million (estimated from sau.com)

Per_Catch <- (Total_Trans_Catch/Global_Catch)*100
...
Transboundary_Spp <- Tresholded_T_Data %>% 
  ...
  mutate(
    ...
    Weight_Spp = ifelse(n_trans_spp/Size_EEZ < 1,1, n_trans_spp/Size_EEZ),
    Weight_Bins = cut(Weight_Spp,breaks = 5),
    Weight_Bins_Plot = cut(Weight_Spp,breaks = 5,
                    labels = c('super low', 'low','median', 'high','super high')
                    )
  )
...
Rev_Breaks <- c(-1,10,100,1000,5000,10000)

Revenue_Country <- sau_eez_fes %>% 
  group_by(fishing_entity_id, fishing_entity) %>% 
  summarise(n=n()) %>% # Get all of the fishing entity and id
  select(-n) %>% 
  full_join(Tresholded_T_Data,
            by = "fishing_entity") %>% 
  ...
  left_join(SAU_Revenue_Data,
            by = c("fishing_entity","fishing_entity_id","taxon_key")) %>% 
  group_by(fishing_entity) %>% # Estimate the amount of dollars of each nation 
  summarise(Total_USD = (sum(mean_value,na.rm=T))/1000000) %>% # Million of USD
  ungroup() %>% 
  # left_join(EEZ_Area, 
            # by = "eez_name") %>% 
  mutate(
    Bins = cut(Total_USD,breaks = Rev_Breaks, dig.lab = 5 ),
    Bins_Plot = cut(Total_USD,breaks = Rev_Breaks,
                    labels = c('super low', 'low','median','high','super high'),
                    dig.lab = 5 # Removes scientific notaion
    )#,
     # Include the weighted method for supplementals
  #   Weight_Rev = Total_USD/Size_EEZ,
  #   Weight_Bins = cut(Weight_Rev,breaks = 5),
  #   Weight_Bins_Plot = cut(Weight_Rev,breaks = 5,
  #                   labels = c('super low', 'low','median', 'high','super high'))
  # )
  ) %>% 
  filter(Total_USD > 0)
...
# Data per Territory
# Transboundary_Jess <- Tresholded_T_Data %>% 
#   group_by(fishing_entity) %>% 
#   summarise(
#     n_trans_spp = length(unique(taxon_key)),
#     # n_neighbours = length(unique(nt_fishing_entity)),
#     # trans_Rrate = n_trans_spp/n_neighbours
#   )

# write_csv(Transboundary_Jess,
#           "transboundary_species_dec19.csv")
...
# Contienent level
Continent_Data <- Revenue_Country %>% 
  left_join(Catch_Country,
            by = "fishing_entity") %>% 
  left_join(UN_Regions,
            by = "fishing_entity") %>% 
...
Continent_Data_Trans <- Tresholded_T_Data %>% 
  left_join(UN_Regions,
            by = "fishing_entity") %>% 
  mutate(georegion = ifelse(is.na(georegion), "Other",paste(georegion))) %>% 
  group_by(georegion) %>% 
  ...
  left_join(Continent_Data) 

### Catch Data

# Catch_Country

Catch_Breaks <- c(-1,10,100,1000,5000,10000)

Catch_Country <- SAU_Catch_Data %>%
  group_by(eez_name) %>%
  summarise(eez_catch = round(sum(total_catch,na.rm=T),2)) %>%
  left_join(Clean_sau_eez_fes,
            by = "eez_name") %>%
  group_by(fishing_entity) %>%
  summarise(fishing_entity_catch = sum(eez_catch,na.rm=T)/1000) %>% # thousand tones
  mutate(
    # fishing_entity = ifelse(fishing_entity == "USA","United States of America",
    #                                ifelse(fishing_entity == "Russian Federation","Russia",fishing_entity)),
    Bins = cut(fishing_entity_catch,breaks = Catch_Breaks),
    Bins_Plot = cut(fishing_entity_catch,breaks = Catch_Breaks,
                    labels = c('super low', 'low','median','high','super high'))
  ) %>% 
  filter(fishing_entity_catch > 0)

### Revenue Data
Subregion_Data <- Revenue_Country %>% 
  left_join(Catch_Country,
            by = "fishing_entity") %>% 
  left_join(UN_Regions,
            by = "fishing_entity") %>% 
  mutate(sub_region = ifelse(is.na(sub_region), "Other",paste(sub_region))) %>% 
  left_join(Regional_Area, 
            by = "sub_region") %>% 
  mutate(
    Weighted_Catch = fishing_entity_catch/Size_Region,
    Weighted_USD = Total_USD/Size_Region
    ) %>% 
  group_by(sub_region) %>% 
...
Subregion_Data_Trans <- Tresholded_T_Data %>% 
  left_join(UN_Regions,
            by = "fishing_entity") %>% 
  mutate(sub_region = ifelse(is.na(sub_region), "Other",paste(sub_region))) %>% 
  group_by(sub_region) %>% 
  ...
  left_join(Subregion_Data) %>% 
  mutate(id = seq(1,nrow(.)))

# Fix names for plot
Subregion_Data_Trans$sub_region <- Subregion_Data_Trans$sub_region %>% 
  gsub("Northern","N.",.) %>% 
  gsub("Southern","S.",.) %>% 
  gsub("Western","W.",.) %>% 
  gsub("Eastern","E.",.)
...
number_of_bar <- nrow(Subregion_Data_Trans)

# Label info Rev
label_data_rev <- Subregion_Data_Trans %>% 
  arrange(Total_USD_sum) %>% # soo it looks like a nice snail :)
  mutate(
    id = seq(1,nrow(Subregion_Data_Trans)),
  ...
  mutate(
    ...
    sub_region_n = paste(sub_region," (n=",n_trans_spp,")",sep = "")
  )

# Label info Catch
label_data_catch <- Subregion_Data_Trans %>% 
  arrange(fishing_entity_catch_sum) %>% # soo it looks like a nice snail :)
  mutate(
    id = seq(1,nrow(Subregion_Data_Trans)),
    angle = 90 -360 * (id-0.5)/number_of_bar,
    hjust = ifelse(angle < -90,1,0),
...
# Label info Weight Catch
label_data_w_catch <- Subregion_Data_Trans %>% 
  arrange(Weighted_Catch_sum) %>% # soo it looks like a nice snail :)
  mutate(
    id = seq(1,nrow(Subregion_Data_Trans)),
    angle = 90 -360 * (id-0.5)/number_of_bar,
    hjust = ifelse(angle < -90,1,0),
...
# Label info Weight Catch
label_data_w_rev <- Subregion_Data_Trans %>% 
  arrange(Weighted_USD_sum) %>% # soo it looks like a nice snail :)
  mutate(
    id = seq(1,nrow(Subregion_Data_Trans)),
    angle = 90 -360 * (id-0.5)/number_of_bar,
    hjust = ifelse(angle < -90,1,0),
...
World_sf_si <- World_sf %>%
  ...
  left_join(Transboundary_Spp, by ="eez_name")# %>% 
# filter(!is.na(n_trans_spp))

# Total catch/Revenue shapefile
World_sf_si_land <- World_land_sf %>%
  st_transform(crs = 4326) %>% # 4326
  st_simplify(preserveTopology = TRUE, dTolerance = 0.1) %>% 
  left_join(Revenue_Country, by ="fishing_entity")# %>% 
...
```{r Territory_Map_Round_Revenue, eval =F, echo =F}
...
Bins_Rev <- Revenue_Country %>% 
  arrange(Bins) %>% 
  pull(Bins) %>% 
  unique()

Labels_Trans <- c("(1-29]",paste(Bins_Trans[2:5]))
Labels_Rev <- c("(1-10]",paste(Bins_Rev[2:5]))
  ...
  Map <- ggplot() +
    ...
    annotate("text", 
             ...
             hjust=0) +
    theme(
      legend.box.margin = margin(t = -54, l = 130),
...
# Set plot limits
max_lim <- max(Subregion_Data_Trans$Total_USD_sum) +10

Rev_Plot <- ggplot(
  # Regional plots
  data = Subregion_Data_Trans, 
  aes(
    x=reorder(id,Total_USD_sum),
    y=Total_USD_sum, 
    fill=Total_USD_sum
  )
) +    
  ...
  # Regional averages
  geom_point(data = Subregion_Data_Trans,
             aes(
               x=reorder(id,Total_USD_sum),
               y=Total_USD_m
             ),
  ...
  geom_errorbar(
    aes(
      ymin=ifelse(Total_USD_m-Total_USD_sd <= 0,0,Total_USD_m-Total_USD_sd), # sets lower to 0
      ymax=Total_USD_m+Total_USD_sd
    ), 
    width=.2,
...
### Catch plot
###____________###
max_lim <- max(Subregion_Data_Trans$fishing_entity_catch_sum) +1

Catch_Plot <- ggplot(data = Subregion_Data_Trans, 
                     aes(x=reorder(id,fishing_entity_catch_sum),
                         y=fishing_entity_catch_sum, 
                         fill=fishing_entity_catch_sum
                     )
) +    
  ylim(-2.1, max_lim) +
  geom_bar(stat="identity") +
  ...
  geom_point(data = Subregion_Data_Trans,
             aes(
               x=reorder(id,fishing_entity_catch_sum),
               y=fishing_entity_catch_m
             ),
             colour = "black",
             alpha = 0.5
  ) +
  ...
  geom_errorbar(
    aes(
      ymin=ifelse(fishing_entity_catch_m-fishing_entity_catch_sd <= 0,0,fishing_entity_catch_m-fishing_entity_catch_sd),
      ymax=fishing_entity_catch_m+fishing_entity_catch_sd
    ), 
    width=.2,
    position=position_dodge(.9),
    alpha = 0.5) +
  ...
  geom_text(data=label_data_catch, 
            aes(x=id,
                y=fishing_entity_catch_sum+0.1,
                label=sub_region,
                hjust=hjust), 
            color="black",
            fontface="bold",
...
plot <- ggdraw() +
  ...
  draw_plot_label(label = c("Revenue (USD) | Catch (Tones)","(Million)"), 
                  size = 8,
                  x = c(0.330,0.485), 
                  y = c(0.31,0.29)
  )
...
```{r Territory_Map_Round_Catch, eval =F, echo =F}


# Get Catch data for mapping (Supplemental materials)
Catch_Breaks <- c(-1,10,100,1000,5000,10000)
...
Bins_Catch <- Catch_Country %>% 
  arrange(Bins) %>% 
  pull(Bins) %>% 
  unique()

Labels_Trans <- c("(1-29]",paste(Bins_Trans[2:5]))
Labels_Catch <- c("(1-10]",paste(Bins_Catch[2:5]))
...
World_sf_si_land <- World_land_sf %>%
  st_transform(crs = 4326) %>%
  st_simplify(preserveTopology = TRUE, dTolerance = 0.1) %>% 
  left_join(Catch_Country, by ="fishing_entity")

# Get the NA's to be grey
Grey_land <- World_sf_si_land %>% 
  filter(is.na(Bins_Plot))


# Make map
Map <- ggplot() +
  geom_sf(data = World_sf_si_land,aes(fill = reorder(Bins_Plot,fishing_entity_catch)), colour = "grey80",size = 0.1) +
  geom_sf(data = World_sf_si,aes(fill =  reorder(Bins_Plot,n_trans_spp)),colour = "grey20", size = 0.1) + # n_trans_spp or Bins
  geom_sf(data = Grey_land,aes(fill = Bins),fill = "grey80", colour = "grey50",size = 0.1) + # n_trans_spp or Bins
  scale_fill_manual("",values = c("#5BBCD6",#"#5BBCD6", # blue
                                  "#00A08A",#"#00A08A", #green
...
# Set plot limits
max_lim <- max(Subregion_Data_Trans$Weighted_USD_sum) +0.01

Rev_Plot <- ggplot(
  # Regional plots
  data = Subregion_Data_Trans, 
  aes(
    x=reorder(id,Weighted_USD_sum),
    y=Weighted_USD_sum, 
    fill=Weighted_USD_sum
  )
) +    
  ylim(-0.001, max_lim) +
  geom_bar(stat="identity") +
  ...
  geom_errorbar(
    aes(
      ymin=ifelse(Weighted_USD_m-Weighted_USD_sd <= 0,0,Weighted_USD_m-Weighted_USD_sd), # sets lower to 0
      ymax=Weighted_USD_m+Weighted_USD_sd
    ),
    width=.2,
    position=position_dodge(.9),
    alpha = 0.5) +
  ...
  #              colour = "black", size=0.1, inherit.aes = FALSE) +
  coord_polar(start = 0) + 
  geom_text(data=label_data_w_rev, 
            aes(x=id,
                y=Weighted_USD_sum+0.0001,
                label=sub_region,
                hjust=hjust), 
            color="black",
            fontface="bold",
...
### Catch plot
###____________###
max_lim <- max(Subregion_Data_Trans$Weighted_Catch_sum) +0.001

Catch_Plot <- ggplot(data = Subregion_Data_Trans, 
                     aes(x=reorder(id,Weighted_Catch_sum),
                         y=Weighted_Catch_sum, 
                         fill=Weighted_Catch_sum
                     )
) +    
  ylim(-0.0003, max_lim) +
  geom_bar(stat="identity") +
  ...
  geom_point(data = Subregion_Data_Trans,
             aes(
               x=reorder(id,Weighted_Catch_sum),
               y=Weighted_Catch_m
             ),
             colour = "black",
             alpha = 0.5
  ...
  geom_errorbar(
    aes(
      ymin=ifelse(Weighted_Catch_m-Weighted_Catch_sd <= 0,0,Weighted_Catch_m-Weighted_Catch_sd),
      ymax=Weighted_Catch_m+Weighted_Catch_sd
    ), 
    width=.2,
    position=position_dodge(.9),
    alpha = 0.5) +
  ...
  #              colour = "black", size=0.1, inherit.aes = FALSE) +
  coord_polar(start = 0) + 
  geom_text(data=label_data_w_catch, 
            aes(x=id,
                y=Weighted_Catch_sum+0.0001,
                label=sub_region,
                hjust=hjust), 
            color="black",
            fontface="bold",
...
plot <- ggdraw() +
  ...
  # draw_plot(Map, x = 0, y = 0.45, width = 1, height = 0.58) +
  # Revenue circular
  draw_plot(Rev_Plot, x = 0.2, y = 0.5, width = 0.5, height = 0.4) +
  # Catch circular
  draw_plot(Catch_Plot, x = 0.46, y = 0.5, width = 0.5, height = 0.4) +
  # Labeks
  draw_plot_label(label = c("A", "B"), 
                  size = 20,
                  x = c(0.1, 0.5), 
...
PeRu <- c("Peru", "Russian Federation", "Chile", "USA")

Total_PeRu <- Revenue_Country %>% 
  filter(fishing_entity %in% PeRu) %>% 
  select(1:2)

Rev_PeRu <- sau_eez_fes %>% 
  filter(fishing_entity %in% PeRu) %>% 
  group_by(fishing_entity_id, fishing_entity) %>% 
  summarise(n=n()) %>% # Get all of the fishing entity and id
  select(-n) %>% 
  ...
  top_n(16,mean_value) %>%
  select(2,3,5) %>% 
  mutate(mean_value = mean_value/1000000) %>% # Million of USD
  left_join(Total_PeRu,
            by = "fishing_entity") %>% 
  mutate(Ratio = mean_value/Total_USD*100)
...
### Total from the top 5

Top5 <- c("Peru", "Russian Federation", "Chile", "USA","China")

Total_PeRu <- Revenue_Country %>% 
  filter(fishing_entity %in% PeRu) %>% 
  select(1:2) %>% 
  group_by() %>% 
  summarise(sum(Total_USD)) %>% 
  pull()/79745
...
### Prepare Data
Subregion_Data_stats <- Revenue_Country %>% 
  left_join(Catch_Country,
            by = "fishing_entity") %>% 
  left_join(UN_Regions,
            by = "fishing_entity") %>% 
  mutate(sub_region = ifelse(is.na(sub_region), "Other",paste(sub_region))) %>% 
  select(fishing_entity,sub_region,Total_USD,fishing_entity_catch)

# Revenue ANOVA
Rev_aov <- aov(Total_USD~sub_region, data = Subregion_Data_stats)
Rev_aov
summary(Rev_aov)

# Df    Sum Sq Mean Sq F value  Pr(>F)    
# sub_region   16  75628526 4726783    4.91 4.2e-08 ***
# Residuals   157 151140183  962676       
...
# Northern America sig than everyone except for Eastern Asia
# Eastern Asia sig with everyone except with Eastern Europe and North Am.

### Catch ANOVA

Catch_aov <- aov(fishing_entity_catch~sub_region, data = Subregion_Data_stats)
Catch_aov

summary(Catch_aov)

# sub_region   16  50.63   3.164   3.767 7.2e-06 ***
# Residuals   155 130.22   0.840                    
# ---
...
Status_Plot_Data <- tibble(
  ...
  observation = rep(unique(Status_Data_Plot$observation),153)
) %>% 
  left_join(UN_Regions) %>% 
...
Status_Data_Stats <- Status_Data_Plot %>% 
  filter(observation != "No Category")


# Differences between groups
Stat_con_aov <- aov(value~group, data = Status_Data_Stats)
Stat_con_aov
summary(Stat_con_aov)

#  Df Sum Sq Mean Sq F value Pr(>F)
# group         5   3148   629.6    1.24  0.289 # NO SIG
# Residuals   443 224996   507.9    

Stat_Cat_aov <- aov(value~observation, data = Status_Data_Stats)
Stat_Cat_aov
summary(Stat_Cat_aov)
#              Df Sum Sq Mean Sq F value Pr(>F)    
# observation   2 134952   67476   322.9 <2e-16 ***
# Residuals   446  93192     209    
...
Mackerel_Rev <- SAU_Revenue_Data %>% 
  filter(taxon_key == 600118,
         fishing_entity %in% Conflict_nations
  ) %>% 
  group_by() %>% 
  summarise(sum(mean_value))/1000000

Mackerel_Catch <- SAU_Catch_Data %>% 
  left_join(Clean_sau_eez_fes) %>% 
  filter(taxon_key == 600118,
         fishing_entity %in% Conflict_nations
  ) %>% 
  group_by() %>% 
  summarise(sum(total_catch))/1000
...
Species_Data <- Tresholded_T_Data %>% 
  ...
  mutate(DemersPelag = ifelse(is.na(DemersPelag),"Other",
                              ifelse(DemersPelag %in% c("bathypelagic","benthopelagic"), "bathy-bento-pelagic",paste(DemersPelag)
                              )
  ),
...
Top_5_Ecosystem <- Species_Data %>% 
  rename(taxon_name = individual) %>% 
  group_by(group) %>% 
  top_n(5,value) 

# If you want to show revenue
# Country_Spp_Data <- sau_eez_fes %>% 
#   group_by(fishing_entity_id,fishing_entity) %>% 
#   summarise(n=n()) %>% 
#   select(-n) %>% 
#   left_join(SAU_Revenue_Data) %>% 
#   left_join(Tresholded_T_Data) %>% 
#   left_join(Exploited_Species) %>% 
#   left_join(Top_5_Ecosystem) %>% # includes the groups
#   filter(taxon_name %in% Top_5_Ecosystem$taxon_name) %>% 
#   group_by(fishing_entity,group) %>% 
#   summarise(top_catch = sum(mean_value)/1000000) %>%  # Million
#   group_by(group) %>% 
#   top_n(5)
...
Country_Spp_Data <- Tresholded_T_Data %>%
  ...
  left_join(Top_5_Ecosystem) %>% # includes the groups
  filter(taxon_name %in% Top_5_Ecosystem$taxon_name) %>%
  left_join(SAU_Catch_Data,
            by = c("eez_name","taxon_key")
  ) %>%
  group_by(fishing_entity,group) %>%
  summarise(top_catch = sum(total_catch)) %>%
  group_by(group) %>%
  top_n(5) %>% 
  mutate(
...
save_plot(
  paste(Figures_Path,"spp_circular_catch_country.png",sep=""),
  # "spp_plot.png",
  spp_plot,
  base_height = 14,
  base_width = 14
...
# Differences between groups
Stat_spp_aov <- aov(value~group, data = Species_Data)
Stat_spp_aov
summary(Stat_spp_aov)
# Df Sum Sq Mean Sq F value   Pr(>F)    
# group         5  24836    4967   14.67 5.52e-11 *** (YUP)
# Residuals   109  36915     339                     
...
for(i in 1:length(Area_Index_Level)){
  ...
  p[[i]] <- ggplot(data) +
    ...
    labs(
      ...
      y = "Frequency of EEZs",
      title= paste((Area_Index_Level[i]*100),"% area treshold"," (Median ",Median_data,")",sep="")
    ) +
...
Status_Per_Species <- Tresholded_T_Data %>% 
  ...
  mutate(Categories = ifelse(status %in% c("Developing","Rebuilding"),"Category A",
